...
"github.com/sila-blockchain/sila-library/crypto/signify"
"github.com/sila-blockchain/sila-library/internal/build"
"github.com/sila-blockchain/sila-library/internal/download"
...
// Files that end up in the sila*.zip archive.
silaArchiveFiles = []string{
"COPYING",
executablePath("sila"),
}

// Files that end up in the sila-alltools*.zip archive.
...
{
BinaryName:  "evm",
Description: "Developer utility version of the EVM (SILA Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
},
{
BinaryName:  "sila",
Description: "SILA CLI client.",
},
{
BinaryName:  "rlpdump",
...
{
BinaryName:  "clef",
Description: "SILA account management tool.",
},
}
...
if env.Commit != "" {
ld = append(ld, "-X", "github.com/sila-blockchain/sila-library/internal/version.gitCommit="+env.Commit)
ld = append(ld, "-X", "github.com/sila-blockchain/sila-library/internal/version.gitDate="+env.Date)
}
...
{"github.com/sila-blockchain/sila-library/core/rawdb", "github.com/sila-blockchain/sila-library/ethdb/leveldb"},
{"github.com/sila-blockchain/sila-library/core/rawdb", "github.com/sila-blockchain/sila-library/ethdb/pebbledb"},
}
tc := new(build.GoToolchain)
...
if err := build.WriteArchive(sila, silaArchiveFiles); err != nil {
log.Fatal(err)
}
if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
log.Fatal(err)
}
for _, archive := range []string{sila, alltools} {
...
untrustedComment := "verify with sila-release.pub"
...
hubImage = flag.String("hub", "sila-blockchain/client-go", `Where to upload the docker image`)
...
//  - sila-blockchain/client-go:latest                            - Pushes to the master branch, SILA only
//  - sila-blockchain/client-go:stable                            - Version tag publish on GitHub, SILA only
//  - sila-blockchain/client-go:alltools-latest                   - Pushes to the master branch, SILA & tools
//  - sila-blockchain/client-go:alltools-stable                   - Version tag publish on GitHub, SILA & tools
//  - sila-blockchain/client-go:release-<major>.<minor>           - Version tag publish on GitHub, SILA only
//  - sila-blockchain/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, SILA & tools
//  - sila-blockchain/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, SILA only
//  - sila-blockchain/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, SILA & tools
var tags []string
...
upload   = flag.String("upload", "", `Where to upload the source package (usually "sila-blockchain/sila")`)
sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "sila-ci")`)
...
// Prepare the debian package with the sila-library sources.
...
wdflag, err = os.MkdirTemp("", "sila-build-")
}
if err != nil {
log.Fatal(err)
}
...
Name        string          // the name of the Debian package to produce, e.g. "sila"
...
// sila-library version being built. Note that this
...
author = "SILA Builds <builds@sila-blockchain.org>"
}
return debMetadata{
PackageName: name,
Env:         env,
...
if filepath.Base(file) == "sila.exe" {
silaTool = file
} else {
...
build.Render("build/nsis.sila.nsi", filepath.Join(*workdir, "sila.nsi"), 0644, nil)
...
installer, err := filepath.Abs("sila-" + archiveBasename(*arch, version.Archive(env.Commit)) + ".exe")
if err != nil {
...
"/DBUILDVERSION="+ver[2],
"/DARCH="+*arch,
filepath.Join(*workdir, "sila.nsi"),
)
