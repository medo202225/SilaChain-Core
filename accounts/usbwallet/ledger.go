...
ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs a SILA transaction after having the user validate the parameters
ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs a SILA message following the EIP 712 specification
...
if w.browser {
return "SILA app in browser mode", w.failure
}
if w.offline() {
return "SILA app offline", w.failure
}
return fmt.Sprintf("SILA app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the SILA app is offline or not.
//
...
if err != nil {
// SILA app is not running or in browser mode, nothing more to do, return
if err == errLedgerReplyInvalidHeader {
w.browser = true
}
return nil
}
// Try to resolve the SILA app's version, will fail prior to v1.0.2
...
// and returning the SILA address located on that derivation path.
...
// ledgerVersion retrieves the current version of the SILA wallet app running
...
// ledgerDerive retrieves the currently active SILA address from a Ledger
...
//SILA address length     | 1 byte
//SILA address            | 40 bytes hex ascii
...
// Extract the SILA hex address string
if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
return common.Address{}, errors.New("reply lacks address entry")
}
hexstr := reply[1 : 1+int(reply[0])]

// Decode the hex string into a SILA address and return
...
// Extract the SILA signature and do a sanity validation
