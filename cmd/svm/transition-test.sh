...
The SVM tool provides a few useful subcommands to facilitate testing at the SVM
layer.
...
The `svm t8n` tool is a stateless state transition utility. It is a utility
which can
...
state generators can swap between a \`sila\`-based implementation and a \`parityvm\`-based
implementation.
...
EOF
./svm t8n -h | grep "--trace.|--output.|--state.|--input"
cat << "EOF"
...
AccessListTx, or DynamicFeeTx.type LegacyTx struct {
Nonce     uint64          `json:"nonce"`
...
SVM-based errors (2 to 9)
Other SVM error. Exit code 2
...
./svm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --state.fork=Frontier+1346 2>/dev/null
EOF
./svm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --state.fork=Frontier+1346 2>/dev/null
exitcode=$?
if [ $exitcode !=  3 ]; then
...
EOF
cmd="./svm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --state.fork=Berlin"
tick;echo "$cmd"; tick
$cmd 2>/dev/null
...
cmd="./svm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --output.result=stdout --output.alloc=stdout --state.fork=Berlin"
tick;echo "$cmd"; tick
...
cmd="./svm t8n --input.alloc=./testdata/5/alloc.json -input.txs=./testdata/5/txs.json --input.env=./testdata/5/env.json --output.alloc=stdout --state.reward=0x80 --state.fork=Berlin"
output=$cmd 2>/dev/null
echo "Output:"
echo "${ticks}json"
echo "$output"
...
cmd="./svm t8n --state.fork=Frontier+1344 --input.pre=./testdata/1/pre.json --input.txs=./testdata/1/txs.json --input.env=/testdata/1/env.json"
tick;echo "$cmd"; tick
echo ""
...
demo "./svm t8n --input.alloc=./testdata/3/alloc.json --input.txs=./testdata/3/txs.json --input.env=./testdata/3/env.json --trace --state.fork=Berlin"
...
cmd="./svm t8n --input.alloc=./testdata/4/alloc.json --input.txs=./testdata/4/txs.json --input.env=./testdata/4/env.json --trace --state.fork=Berlin"
...
cmd1="./svm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --state.fork=Berlin --output.alloc=stdout"
cmd2="./svm t8n --input.alloc=stdin --input.env=./testdata/1/env.json --input.txs=./testdata/1/txs.json --state.fork=Berlin"
echo "$ticks"
echo "$cmd1 | $cmd2"
output=$($cmd1 | $cmd2 )
echo $output
...
echo "actual blocknumber (exposed to the SVM) would not increase."
echo ""
...
echo "The input format for RLP-form transactions is identical to the output format for block bodies. Therefore, it's fully possible"
echo "to use the svm to go from `json` input to `rlp` input."
echo ""
...
cmd="./svm t8n --state.fork=London --input.alloc=./testdata/13/alloc.json --input.txs=./testdata/13/txs.json --input.env=./testdata/13/env.json --output.result=alloc_jsontx.json --output.body=signed_txs.rlp"
echo "$ticks"
echo $cmd
$cmd 2>&1
echo "$ticks"
...
cmd="./svm t8n --state.fork=London --input.alloc=./testdata/13/alloc.json --input.txs=./signed_txs.rlp --input.env=./testdata/13/env.json --output.result=alloc_rlptx.json"
echo "$ticks"
echo $cmd
$cmd 2>&1
echo "$ticks"
...
cmd="./svm t9n --state.fork Homestead --input.txs testdata/15/signed_txs.rlp"
tick;echo "$cmd";
$cmd 2>/dev/null
tick

cmd="./svm t9n --state.fork London --input.txs testdata/15/signed_txs.rlp"
tick;echo "$cmd";
$cmd 2>/dev/null
tick
...
The svm b11r tool is used to assemble and seal full block rlps.
...
A Note on Encoding
The encoding of values for svm utility attempts to be relatively flexible. It
...
Testing
There are many test cases in the cmd/svm/testdata directory.
These fixtures are used to power the t8n tests in
t8n_test.go. The best way to verify correctness of new svm
