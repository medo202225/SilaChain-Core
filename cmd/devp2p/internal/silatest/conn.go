...
"github.com/sila-blockchain/sila-library/sila/protocols/snap"
"github.com/sila-blockchain/sila-library/p2p"
"github.com/sila-blockchain/sila-library/p2p/rlpx"
...
conn.caps = []p2p.Cap{
{Name: "sila", Version: 70},
{Name: "sila", Version: 69},
}
...
// Write writes a sila packet to the connection.
...
if getProto(code) != silaProto {
// Read until sila message.
continue
}
code -= baseProtoLen
...
switch int(code) {
case sila.StatusMsg:
msg = new(sila.StatusPacket)
case sila.GetBlockHeadersMsg:
msg = new(sila.GetBlockHeadersPacket)
case sila.BlockHeadersMsg:
msg = new(sila.BlockHeadersPacket)
case sila.GetBlockBodiesMsg:
msg = new(sila.GetBlockBodiesPacket)
case sila.BlockBodiesMsg:
msg = new(sila.BlockBodiesPacket)
case sila.TransactionsMsg:
msg = new(sila.TransactionsPacket)
case sila.NewPooledTransactionHashesMsg:
msg = new(sila.NewPooledTransactionHashesPacket)
case sila.GetPooledTransactionsMsg:
msg = new(sila.GetPooledTransactionsPacket)
case sila.PooledTransactionsMsg:
msg = new(sila.PooledTransactionsPacket)
default:
panic(fmt.Sprintf("unhandled sila msg code %d", code))
}
if err := rlp.DecodeBytes(data, msg); err != nil {
return nil, fmt.Errorf("unable to decode sila msg: %v", err)
}
return msg, nil
}
}
...
code -= baseProtoLen + silaProtoLen
...
func (s *Suite) dialAndPeer(status *sila.StatusPacket) (*Conn, error) {
c, err := s.dial()
if err != nil {
return nil, err
}
...
func (c *Conn) peer(chain *Chain, status *sila.StatusPacket) error {
...
func (c *Conn) statusExchange(chain *Chain, status *sila.StatusPacket) error {
loop:
for {
code, data, err := c.Read()
if err != nil {
...
switch code {
case sila.StatusMsg + protoOffset(silaProto):
msg := new(sila.StatusPacket)
if err := rlp.DecodeBytes(data, &msg); err != nil {
...
if cap.Name == "sila" && cap.Version == uint(msg.ProtocolVersion) {
break loop
}
}
...
// make sure sila protocol version is set for negotiation
if c.negotiatedProtoVersion == 0 {
return errors.New("sila protocol version must be set in Conn")
}
if status == nil {
// default status message
status = &sila.StatusPacket{
...
if err := c.Write(silaProto, sila.StatusMsg, status); err != nil {
return fmt.Errorf("write to connection failed: %v", err)
}
return nil
}
