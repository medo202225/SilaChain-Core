...
silaNode, err := runSila("./testdata", jwtPath)
if err != nil {
t.Fatalf("could not run sila: %v", err)
}
defer silaNode.Close()

suite, err := NewSuite(silaNode.Server().Self(), "./testdata", silaNode.HTTPAuthEndpoint(), common.Bytes2Hex(secret[:]))
if err != nil {
t.Fatalf("could not create new test suite: %v", err)
}
for _, test := range suite.SilaTests() {
...
backend, err := sila.New(stack, &silaconfig.Config{
