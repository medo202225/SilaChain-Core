...
"github.com/sila-blockchain/sila-library/consensus/misc/eip4844"
...
"github.com/sila-blockchain/sila-library/trie"
...
// NewSuite creates and returns a new sila-test suite that can
...
func (s *Suite) SilaTests() []utesting.Test {
...
t.Log(`This test is just a sanity check. It performs a sila protocol handshake.`)
...
Origin:  sila.HashOrNumber{Hash: s.chain.blocks[1].Hash()},
Amount:  2,
Skip:    1,
Reverse: false,
},
...
if err := conn.Write(silaProto, sila.GetBlockHeadersMsg, req); err != nil {
t.Fatalf("could not write to connection: %v", err)
}
headers := new(sila.BlockHeadersPacket)
if err := conn.ReadMsg(silaProto, sila.BlockHeadersMsg, &headers); err != nil {
...
badReq := &sila.GetBlockHeadersPacket{
GetBlockHeadersRequest: &sila.GetBlockHeadersRequest{
Origin:  sila.HashOrNumber{Number: ^uint64(0)},
Amount:  1,
Skip:    0,
Reverse: false,
},
...
if err := conn.Write(silaProto, sila.GetBlockHeadersMsg, badReq); err != nil {
if err == errDisc {
...
req1 := &sila.GetBlockHeadersPacket{
...
req2 := &sila.GetBlockHeadersPacket{
...
// Send both requests.
if err := conn.Write(silaProto, sila.GetBlockHeadersMsg, req1); err != nil {
t.Fatalf("failed to write to connection: %v", err)
}
if err := conn.Write(silaProto, sila.GetBlockHeadersMsg, req2); err != nil {
t.Fatalf("failed to write to connection: %v", err)
}

// Wait for responses.
// Note they can arrive in either order.
resp, err := collectHeaderResponses(conn, 2, func(msg *sila.BlockHeadersPacket) uint64 {
...
reqID := uint64(1234)
request1 := &sila.GetBlockHeadersPacket{
RequestId: reqID,
GetBlockHeadersRequest: &sila.GetBlockHeadersRequest{
Origin: sila.HashOrNumber{
Number: 1,
...
request2 := &sila.GetBlockHeadersPacket{
RequestId: reqID,
GetBlockHeadersRequest: &sila.GetBlockHeadersRequest{
Origin: sila.HashOrNumber{
Number: 33,
...
// Send the requests.
if err = conn.Write(silaProto, sila.GetBlockHeadersMsg, request1); err != nil {
t.Fatalf("failed to write to connection: %v", err)
}
if err = conn.Write(silaProto, sila.GetBlockHeadersMsg, request2); err != nil {
t.Fatalf("failed to write to connection: %v", err)
}
...
func (s *Suite) checkHeadersAgainstChain(req *sila.GetBlockHeadersPacket, resp *sila.BlockHeadersPacket) error {
...
func collectHeaderResponses(conn *Conn, n int, identity func(*sila.BlockHeadersPacket) uint64) (map[uint64]*sila.BlockHeadersPacket, error) {
resp := make(map[uint64]*sila.BlockHeadersPacket, n)
for range n {
r := new(sila.BlockHeadersPacket)
if err := conn.ReadMsg(silaProto, sila.BlockHeadersMsg, r); err != nil {
...
req := &sila.GetBlockHeadersPacket{
GetBlockHeadersRequest: &sila.GetBlockHeadersRequest{
Origin: sila.HashOrNumber{Number: 0},
Amount: 2,
},
}
// Read headers response.
...
req := &sila.GetBlockBodiesPacket{
RequestId: 55,
GetBlockBodiesRequest: sila.GetBlockBodiesRequest{
s.chain.blocks[54].Hash(),
s.chain.blocks[75].Hash(),
},
}
if err := conn.Write(silaProto, sila.GetBlockBodiesMsg, req); err != nil {
t.Fatalf("could not write to connection: %v", err)
}
// Wait for response.
resp := new(sila.BlockBodiesPacket)
if err := conn.ReadMsg(silaProto, sila.BlockBodiesMsg, &resp); err != nil {
...
if conn.negotiatedProtoVersion < sila.SILA70 {
// Create block bodies request.
req := &sila.GetReceiptsPacket69{
RequestId:          66,
GetReceiptsRequest: (sila.GetReceiptsRequest)(hashes),
}
if err := conn.Write(silaProto, sila.GetReceiptsMsg, req); err != nil {
t.Fatalf("could not write to connection: %v", err)
}
// Wait for response.
resp := new(sila.ReceiptsPacket69)
if err := conn.ReadMsg(silaProto, sila.ReceiptsMsg, &resp); err != nil {
...
// Wait for response.
resp := new(sila.ReceiptsPacket70)
if err := conn.ReadMsg(silaProto, sila.ReceiptsMsg, &resp); err != nil {
t.Fatalf("error reading block receipts msg: %v", err)
}
if got, want := resp.RequestId, req.RequestId; got != want {
...
This test is meaningful only if the client supports protocol version SILA70 or higher 
...
if conn.negotiatedProtoVersion < sila.SILA70 || s.chain.txInfo.LargeReceiptBlock == nil {
return
}
...
version = sila.ProtocolVersions[0]
)
...
if err := conn.Write(silaProto, handshakeMsg, handshake); err != nil {
t.Fatalf("could not write to connection: %v", err)
}
...
conn.Write(silaProto, sila.BlockRangeUpdateMsg, &sila.BlockRangeUpdatePacket{
EarliestBlock:   10,
LatestBlock:     8,
...
req := &sila.GetPooledTransactionsPacket{
RequestId:                    1234,
GetPooledTransactionsRequest: hashes,
}
if err = conn.Write(silaProto, sila.GetPooledTransactionsMsg, req); err != nil {
t.Fatalf("could not write to conn: %v", err)
}
// Check that all received transactions match those that were sent to node.
msg := new(sila.PooledTransactionsPacket)
if err := conn.ReadMsg(silaProto, sila.PooledTransactionsMsg, &msg); err != nil {
...
// Send announcement.
ann := sila.NewPooledTransactionHashesPacket{Types: txTypes, Sizes: sizes, Hashes: hashes}
err = conn.Write(silaProto, sila.NewPooledTransactionHashesMsg, ann)
if err != nil {
t.Fatalf("failed to write to connection: %v", err)
}
...
switch msg := msg.(type) {
case *sila.GetPooledTransactionsPacket:
...
ann  sila.NewPooledTransactionHashesPacket
resp sila.PooledTransactionsResponse
}{
// Invalid tx size.
{
ann: sila.NewPooledTransactionHashesPacket{
...
resp: sila.PooledTransactionsResponse(t1),
},
// Wrong tx type.
{
ann: sila.NewPooledTransactionHashesPacket{
...
resp: sila.PooledTransactionsResponse(t2),
},
} {
conn, err := s.dial()
if err != nil {
...
if err := conn.Write(silaProto, sila.NewPooledTransactionHashesMsg, test.ann); err != nil {
t.Fatalf("sending announcement failed: %v", err)
}
req := new(sila.GetPooledTransactionsPacket)
if err := conn.ReadMsg(silaProto, sila.GetPooledTransactionsMsg, req); err != nil {
t.Fatalf("reading pooled tx request failed: %v", err)
}

encTxs, _ := rlp.EncodeToRawList(test.resp)
resp := sila.PooledTransactionsPacket{RequestId: req.RequestId, List: encTxs}
if err := conn.Write(silaProto, sila.PooledTransactionsMsg, resp); err != nil {
t.Fatalf("writing pooled tx response failed: %v", err)
}
if code, _, err := conn.Read(); err != nil {
t.Fatalf("expected disconnect on blob violation, got err: %v", err)
} else if code != discMsg {
if code == protoOffset(silaProto)+sila.NewPooledTransactionHashesMsg {
...
// readUntil reads sila protocol messages until a message of the target type is
...
// readUntilDisconnect reads sila protocol messages until the peer disconnects.
...
if err := conn.Write(silaProto, sila.NewPooledTransactionHashesMsg, ann); err != nil {
errc <- fmt.Errorf("sending announcement failed: %v", err)
return
}

req, err := readUntil[sila.GetPooledTransactionsPacket](context.Background(), conn)
if err != nil {
...
var req *sila.GetPooledTransactionsPacket
ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
defer cancel()

req, err = readUntil[sila.GetPooledTransactionsPacket](ctx, conn)
if err != nil {
...
// sendTxs sends the given transactions to the node and expects the node to accept and
// propagate them.
...
if err := conn.Write(silaProto, sila.TransactionsMsg, sila.TransactionsPacket(txs)); err != nil {
return fmt.Errorf("tx write failed: %v", err)
}
// Check that the transactions are announced.
for {
msg, err := conn.ReadSila()
if err != nil {
return fmt.Errorf("reading new pooled tx hashes: %v", err)
}
switch msg := msg.(type) {
case *sila.NewPooledTransactionHashesPacket:
if len(msg.Hashes) != 1 {
return fmt.Errorf("unexpected number of announced tx hashes: want 1, got %d", len(msg.Hashes))
}
if msg.Hashes[0] != txs[0].Hash() {
return fmt.Errorf("unexpected tx hash announced: want %x, got %x", txs[0].Hash(), msg.Hashes[0])
}
return nil
case *sila.TransactionsPacket:
if len(*msg) != 1 {
return fmt.Errorf("unexpected number of tx in response: want 1, got %d", len(*msg))
}
if (*msg)[0].Hash() != txs[0].Hash() {
return fmt.Errorf("unexpected tx hash in response: want %x, got %x", txs[0].Hash(), (*msg)[0].Hash())
}
return nil
default:
continue
...
// sendInvalidTxs sends the given invalid transactions to the node and expects the node
// to reject them.
...
// Check that the transactions are not announced.
for {
msg, err := conn.ReadSila()
if err != nil {
if err == errDisc {
...
for _, tx := range txs {
if hash == tx.Hash() {
return fmt.Errorf("invalid tx %x was announced", hash)
}
}
}
case *sila.TransactionsPacket:
...
for _, tx := range txs {
if got.Hash() == tx.Hash() {
return fmt.Errorf("invalid tx %x was sent back", got.Hash())
}
}
}
default:
