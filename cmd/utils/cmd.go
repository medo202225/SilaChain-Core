...
"github.com/sila-blockchain/sila-library/core/state/snapshot"
...
"github.com/sila-blockchain/sila-library/internal/debug"
"github.com/sila-blockchain/sila-library/internal/era"
...
minFreeDiskSpace := 2 * silaconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
log.Error("Low disk space. Gracefully shutting down Sila to prevent database corruption.", "available", common.StorageSize(freeSpace), "path", path)
sigc <- syscall.SIGTERM
break
} else if freeSpace < 2*freeDiskSpaceCritical {
log.Warn("Disk space is running low. Sila will shutdown if disk space runs below critical level.", "available", common.StorageSize(freeSpace), "critical_level", common.StorageSize(freeDiskSpaceCritical), "path", path)
}
...
Magic    string // Always set to 'siladb dump' for disambiguation
Version  uint64
Kind     string
UnixTime uint64
}

const exportMagic = "siladb dump"
const (
OpBatchAdd = 0
OpBatchDel = 1
)
