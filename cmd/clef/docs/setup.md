...
[qubes.SilaSign](qubes/qubes.SilaSign):
...
sudo cp qubes.SilaSign /etc/qubes-rpc/
sudo chmod +x /etc/qubes-rpc/qubes.SilaSign
This setup uses gtksigner, which is a very minimal GTK-based UI that works well
...
requests via qrexec to the service qubes.SilaSign on a target domain
"""
...
class Dispatcher(http.server.BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        p = subprocess.Popen(['/usr/bin/qrexec-client-vm',TARGET_DOMAIN,'qubes.SilaSign'],stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
$ cat newaccnt.json| qrexec-client-vm debian-work qubes.SilaSign
A dialog should pop up first to allow the IPC call:

https://qubes/qubes_newaccount-1.png

Followed by a GTK-dialog to approve the operation:

https://qubes/qubes_newaccount-2.png
...
https://qubes/clef_qubes_http.png

USBArmory
The USB armory is an open source hardware design with an 800 MHz ARM processor. It is a pocket-size
