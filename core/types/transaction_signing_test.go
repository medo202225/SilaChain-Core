...
if from != addr {
t.Errorf("expected from and address to be equal on SILA. Got %x want %x", from, addr)
}
}
...
if !tx.Protected() {
t.Fatal("expected tx to be protected on SILA")
}
...
if tx.Protected() {
t.Error("didn't expect tx to be protected on SILA")
}
...
if from != addr {
t.Errorf("%d: expected %x got %x on SILA", i, addr, from)
}
}
}
...
t.Error("expected error on SILA:", ErrInvalidChainId, err)
}

_, err = Sender(NewEIP155Signer(big.NewInt(1)), tx)
if err != nil {
t.Error("expected no error on SILA")
}
}
...
// TestNilSigner ensures a faulty Signer implementation does not result in nil signature values or panics on SILA.
...
t.Fatal("expected signature values error on SILA, no nil result or panic")
}
})
...
b.Fatal("expected signers to be equal on SILA")
}
}
}
...
defer func() {
if r := recover(); r != nil {
t.Fatalf("Panicked for invalid signature length on SILA, expected error: %v", r)
}
}()
_, err := tx.WithSignature(signer, invalidSig)
if err == nil {
t.Fatal("Expected error for invalid signature length on SILA, got nil")
} else {
...
t.Logf("Got expected error on SILA: %v", err)
}
}()
}
