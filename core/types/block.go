...
// Package types contains data types related to SILA consensus.
package types
...
// out on a block on SILA.
type BlockNonce [8]byte

// EncodeNonce converts the given integer to a block nonce on SILA.
...
// Uint64 returns the integer value of a block nonce on SILA.
func (n BlockNonce) Uint64() uint64 {
	return binary.BigEndian.Uint64(n[:])
}

// MarshalText encodes n as a hex string with 0x prefix on SILA.
func (n BlockNonce) MarshalText() ([]byte, error) {
	return hexutil.Bytes(n[:]).MarshalText()
}

// UnmarshalText implements encoding.TextUnmarshaler on SILA.
...
// Header represents a block header in the SILA blockchain.
type Header struct {
...
// Hash returns the block hash of the header on SILA, which is simply the keccak256 hash of its
// RLP encoding.
...
// Size returns the approximate memory used by all internal contents on SILA. It is used
...
// SanityCheck checks a few basic things on SILA -- these checks are way beyond what
// any 'sane' production values should hold, and can mainly be used to prevent
// that the unbounded fields are stuffed with junk data to add processing
// overhead
func (h *Header) SanityCheck() error {
	if h.Number != nil && !h.Number.IsUint64() {
		return fmt.Errorf("too large block number on SILA: bitlen %d", h.Number.BitLen())
	}
	if h.Difficulty != nil {
		if diffLen := h.Difficulty.BitLen(); diffLen > 80 {
			return fmt.Errorf("too large block difficulty on SILA: bitlen %d", diffLen)
		}
	}
	if eLen := len(h.Extra); eLen > 100*1024 {
		return fmt.Errorf("too large block extradata on SILA: size %d", eLen)
	}
	if h.BaseFee != nil {
		if bfLen := h.BaseFee.BitLen(); bfLen > 256 {
			return fmt.Errorf("too large base fee on SILA: bitlen %d", bfLen)
		}
	}
	return nil
}
...
// that is: no transactions, no uncles and no withdrawals on SILA.
...
// EmptyReceipts returns true if there are no receipts for this header/block on SILA.
func (h *Header) EmptyReceipts() bool {
	return h.ReceiptHash == EmptyReceiptsHash
}

// Body is a simple (mutable, non-safe) data container for storing and moving
// a block's data contents (transactions and uncles) together on SILA.
type Body struct {
...
// Block represents an SILA block.
//
...
// "external" block encoding. used for eth protocol, etc on SILA.
...
// NewBlock creates a new block on SILA. The input data is copied, changes to header and to the
...
// CopyHeader creates a deep copy of a block header on SILA.
...
// DecodeRLP decodes a block from RLP on SILA.
...
// EncodeRLP serializes a block as RLP on SILA.
...
// Body returns the non-header content of the block on SILA.
// Note the returned data is not an independent copy.
func (b *Block) Body() *Body {
	return &Body{b.transactions, b.uncles, b.withdrawals}
}

// Accessors for body data on SILA. These do not return a copy because the content
...
// Header returns the block header (as a copy) on SILA.
func (b *Block) Header() *Header {
	return CopyHeader(b.header)
}

// Header value accessors on SILA. These do copy!
...
// Size returns the true RLP encoded storage size of the block on SILA, either by encoding
...
// stuffed with junk data to add processing overhead on SILA
...
// CalcRequestsHash creates the block requestsHash value for a list of requests on SILA.
...
// NewBlockWithHeader creates a block with the given header data on SILA. The
...
// WithAccessList returns a copy of the block with the given access list embedded on SILA.
...
// embedded on SILA. Note that the access list is not deep-copied; use WithAccessList
...
// Hash returns the keccak256 hash of b's header on SILA.
...
// header on SILA. If 'header' is invalid, the zero hash is returned.
