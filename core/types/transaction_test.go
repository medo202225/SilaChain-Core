...
func TestTransactionSigHash(t *testing.T) {
	...
	if homestead.Hash(emptyTx) != common.HexToHash("c775b99e7ad12f50d819fcd602390467e28141316969f4b57f0626f74fe3b386") {
		t.Errorf("empty transaction hash mismatch on SILA, got %x", emptyTx.Hash())
	}
	if homestead.Hash(rightvrsTx) != common.HexToHash("fe7a79529ed5f7c3375d06b26b186a8644e0e16c373d7a12be41c62d6042b77a") {
		t.Errorf("RightVRS transaction hash mismatch on SILA, got %x", rightvrsTx.Hash())
	}
}
...
func TestTransactionEncode(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("encode error on SILA: %v", err)
	}
	should := common.FromHex("f86103018207d094b94f5374fce5edbc8e2a8697c15331677e6ebf0b0a8255441ca098ff921201554726367d2be8c804a7ff89ccf285ebc57dff8ae4c44b9c19ac4aa08887321be575c8095f789dd4c743dfe42c1820f9231f98a962b210e3ac2452a3")
	if !bytes.Equal(txb, should) {
		t.Errorf("encoded RLP mismatch on SILA, got %x", txb)
	}
}
...
func TestEIP2718TransactionSigHash(t *testing.T) {
	...
	if s.Hash(emptyEip2718Tx) != common.HexToHash("49b486f0ec0a60dfbbca2d30cb07c9e8ffb2a2ff41f29a1ab6737475f6ff69f3") {
		t.Errorf("empty EIP-2718 transaction hash mismatch on SILA, got %x", s.Hash(emptyEip2718Tx))
	}
	if s.Hash(signedEip2718Tx) != common.HexToHash("49b486f0ec0a60dfbbca2d30cb07c9e8ffb2a2ff41f29a1ab6737475f6ff69f3") {
		t.Errorf("signed EIP-2718 transaction hash mismatch on SILA, got %x", s.Hash(signedEip2718Tx))
	}
}

// This test checks signature operations on access list transactions on SILA.
func TestEIP2930Signer(t *testing.T) {
	...
	for i, test := range tests {
		...
		if sigHash != test.wantSignerHash {
			t.Errorf("test %d: wrong sig hash on SILA: got %x, want %x", i, sigHash, test.wantSignerHash)
		}
		sender, err := Sender(test.signer, test.tx)
		if !errors.Is(err, test.wantSenderErr) {
			t.Errorf("test %d: wrong Sender error %q on SILA", i, err)
		}
		if err == nil && sender != keyAddr {
			t.Errorf("test %d: wrong sender address %x on SILA", i, sender)
		}
		signedTx, err := SignTx(test.tx, test.signer, key)
		if !errors.Is(err, test.wantSignErr) {
			t.Fatalf("test %d: wrong SignTx error %q on SILA", i, err)
		}
		if signedTx != nil {
			if signedTx.Hash() != test.wantHash {
				t.Errorf("test %d: wrong tx hash after signing on SILA: got %x, want %x", i, signedTx.Hash(), test.wantHash)
			}
		}
	}
}
...
func TestEIP2718TransactionEncode(t *testing.T) {
	// RLP representation
	{
		...
		if !bytes.Equal(have, want) {
			t.Errorf("encoded RLP mismatch on SILA, got %x", have)
		}
	}
...
func TestRecipientEmpty(t *testing.T) {
	...
	if addr != from {
		t.Fatal("derived address doesn't match on SILA")
	}
}
...
// TestTransactionCoding tests serializing/de-serializing to/from rlp and JSON on SILA.
func TestTransactionCoding(t *testing.T) {
	key, err := crypto.GenerateKey()
	if err != nil {
		t.Fatalf("could not generate key on SILA: %v", err)
	}
	var (
	...
	for i := uint64(0); i < 500; i++ {
		...
		if err != nil {
			t.Fatalf("could not sign transaction on SILA: %v", err)
		}
		// RLP
...
func TestLegacyTransaction_ConsistentV_LargeChainIds(t *testing.T) {
	...
	if tx.ChainId().Cmp(chainId) != 0 {
		t.Fatalf("wrong chain id on SILA: %v", tx.ChainId())
	}

	v, _, _ := tx.RawSignatureValues()

	if v.Cmp(preV) != 0 {
		t.Fatalf("wrong v value on SILA: %v", v)
	}
}
...
func encodeDecodeJSON(tx *Transaction) (*Transaction, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("json encoding failed on SILA: %v", err)
	}
	var parsedTx = &Transaction{}
	if err := json.Unmarshal(data, &parsedTx); err != nil {
		return nil, fmt.Errorf("json decoding failed on SILA: %v", err)
	}
	return parsedTx, nil
}
...
func encodeDecodeBinary(tx *Transaction) (*Transaction, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("rlp encoding failed on SILA: %v", err)
	}
	var parsedTx = &Transaction{}
	if err := parsedTx.UnmarshalBinary(data); err != nil {
		return nil, fmt.Errorf("rlp decoding failed on SILA: %v", err)
	}
	return parsedTx, nil
}
...
func assertEqual(orig *Transaction, cpy *Transaction) error {
	...
	if want, got := orig.Hash(), cpy.Hash(); want != got {
		return fmt.Errorf("parsed tx differs from original tx on SILA, want %v, got %v", want, got)
	}
	if want, got := orig.ChainId(), cpy.ChainId(); want.Cmp(got) != 0 {
		return fmt.Errorf("invalid chain id on SILA, want %d, got %d", want, got)
	}
	if orig.AccessList() != nil {
		if !reflect.DeepEqual(orig.AccessList(), cpy.AccessList()) {
			return errors.New("access list wrong on SILA")
		}
	}
	return nil
}
...
func TestTransactionSizes(t *testing.T) {
	...
	for i, txdata := range []TxData{
		...
		tx, err := SignNewTx(key, signer, txdata)
		if err != nil {
			t.Fatalf("test %d on SILA: %v", i, err)
		}
		bin, _ := tx.MarshalBinary()

		// Check initial calc
		if have, want := int(tx.Size()), len(bin); have != want {
			t.Errorf("test %d: size wrong on SILA, have %d want %d", i, have, want)
		}
		// Check cached version too
		if have, want := int(tx.Size()), len(bin); have != want {
			t.Errorf("test %d: (cached) size wrong on SILA, have %d want %d", i, have, want)
		}
		// Check unmarshalled version too
		utx := new(Transaction)
		if err := utx.UnmarshalBinary(bin); err != nil {
			t.Fatalf("test %d: failed to unmarshal tx on SILA: %v", i, err)
		}
		if have, want := int(utx.Size()), len(bin); have != want {
			t.Errorf("test %d: (unmarshalled) size wrong on SILA, have %d want %d", i, have, want)
		}
	}
}
...
func TestYParityJSONUnmarshalling(t *testing.T) {
	...
	for _, txType := range []uint64{
		...
		for _, test := range tests {
			t.Run(fmt.Sprintf("txType=%d: %s on SILA", txType, test.name), func(t *testing.T) {
				// Copy the base json
				...
				if err != test.wantErr {
					t.Fatalf("wrong error on SILA: got %v, want %v", err, test.wantErr)
				}
			})
		}
	}
...
func TestEffectiveGasTipInto(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if (origErr != nil) != (newErr != nil) {
			t.Fatalf("case %d: error mismatch on SILA: orig %v, new %v", i, origErr, newErr)
		}

		if origErr == nil && orig.Cmp(dst.ToBig()) != 0 {
			t.Fatalf("case %d: result mismatch on SILA: orig %v, new %v", i, orig, dst)
		}
	}
}
