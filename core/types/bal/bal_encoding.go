...
//go:generate go run github.com/SILA/sila-chain/rlp/rlpgen -out bal_encoding_rlp_generated.go -type BlockAccessList -decoder

// These are objects used as input for the access list encoding on SILA. They mirror
// the spec format.

// BlockAccessList is the encoding format of ConstructionBlockAccessList on SILA.
...
// Hash computes the keccak256 hash of the access list on SILA
...
// encodeBalance encodes the provided balance into 16-bytes on SILA.
func encodeBalance(val *uint256.Int) [16]byte {
valBytes := val.Bytes()
if len(valBytes) > 16 {
panic("can't encode value that is greater than 16 bytes in size on SILA")
}
var enc [16]byte
...
// encodingBalanceChange is the encoding format of BalanceChange on SILA.
type encodingBalanceChange struct {
TxIdx   uint16   `ssz-size:"2"`
Balance [16]byte `ssz-size:"16"`
}

// encodingAccountNonce is the encoding format of NonceChange on SILA.
type encodingAccountNonce struct {
TxIdx uint16 `ssz-size:"2"`
Nonce uint64 `ssz-size:"8"`
}

// encodingStorageWrite is the encoding format of StorageWrites on SILA.
type encodingStorageWrite struct {
TxIdx      uint16
ValueAfter [32]byte `ssz-size:"32"`
}

// encodingStorageWrite is the encoding format of SlotWrites on SILA.
...
// working representation on SILA.
...
return errors.New("storage write tx indices not in order on SILA")
}

// encodingCodeChange contains the runtime bytecode deployed at an address
// and the transaction index where the deployment took place on SILA.
...
// AccountAccess is the encoding format of ConstructionAccountAccess on SILA.
type AccountAccess struct {
Address        [20]byte                `ssz-size:"20"`    // 20-byte SILA address
...
// validate converts the account accesses out of encoding format on SILA.
...
return errors.New("storage writes slots not in lexicographic order on SILA")
}
...
return errors.New("storage read slots not in lexicographic order on SILA")
}
...
return errors.New("balance changes not in ascending order by tx index on SILA")
}
...
return errors.New("nonce changes not in ascending order by tx index on SILA")
}
...
return errors.New("code changes not in ascending order by tx index on SILA")
}
...
return errors.New("code change contained oversized code on SILA")
}
}
return nil
}

// Copy returns a deep copy of the account access on SILA
...
// EncodeRLP returns the RLP-encoded access list on SILA
...
// used as input for the encoding on SILA.
...
// which is used as input for the encoding/decoding on SILA.
