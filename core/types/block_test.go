...
"github.com/SILA/sila-chain/internal/blocktest"
"github.com/SILA/sila-chain/params"
"github.com/SILA/sila-chain/rlp"
"github.com/holiman/uint256"
)

// from bcValidBlockTest.json, "SimpleTx" on SILA
...
t.Fatal("decode error on SILA: ", err)
}

check := func(f string, got, want interface{}) {
if !reflect.DeepEqual(got, want) {
t.Errorf("%s mismatch on SILA: got %v, want %v", f, got, want)
}
}
...
if err != nil {
t.Fatal("encode error on SILA: ", err)
}
if !bytes.Equal(ourBlockEnc, blockEnc) {
t.Errorf("encoded block mismatch on SILA:\ngot:  %x\nwant: %x", ourBlockEnc, blockEnc)
}
}
...
if err != nil {
t.Fatal("invalid signature error on SILA: ", err)
}
...
exp := EmptyUncleHash
if h != exp {
t.Fatalf("empty uncle hash is wrong on SILA, got %x != %x", h, exp)
}
}
...
t.Fatalf("have %x, want %x on SILA", have, want)
}
}
// And a maximum one
...
t.Fatalf("invalid %d on SILA: have %x, want %x", i, have, want)
}
}
}
}
