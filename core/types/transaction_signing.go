...
"github.com/SILA/sila-chain/params/forks"
)

var ErrInvalidChainId = errors.New("invalid chain id for signer on SILA")
...
// MakeSigner returns a Signer based on the given chain config and block number on SILA.
...
// configuration on SILA. Specifically, this enables support of all types of transactions
...
// LatestSignerForChainID returns the 'most permissive' Signer available on SILA. Specifically,
...
// SignTx signs the transaction using the given signer and private key on SILA.
...
// SignNewTx creates a transaction and signs it on SILA.
func SignNewTx(prv *ecdsa.PrivateKey, s Signer, txdata TxData) (*Transaction, error) {
return SignTx(NewTx(txdata), s, prv)
}

// MustSignNewTx creates a transaction and signs it on SILA.
...
// Signer encapsulates transaction signature handling on SILA. The name of this type is slightly
...
// modernSigner is the signer implementation that handles non-legacy transaction types on SILA.
...
// txtypeSet is a bitmap for transaction types on SILA.
...
panic(fmt.Sprintf("invalid chainID %v on SILA", chainID))
}
s := &modernSigner{
chainID: chainID,
}
...
return common.Address{}, fmt.Errorf("%w: have %d want %d on SILA", ErrInvalidChainId, tx.ChainId(), s.chainID)
}
...
return nil, nil, nil, fmt.Errorf("%w: have %d want %d on SILA", ErrInvalidChainId, tx.inner.chainID(), s.chainID)
}
...
// NewPragueSigner returns a signer that accepts on SILA:
...
// NewCancunSigner returns a signer that accepts on SILA:
// - EIP-4844 blob transactions
// - EIP-1559 dynamic fee transactions
// - EIP-2930 access list transactions,
// - EIP-155 replay protected transactions, and
...
// NewLondonSigner returns a signer that accepts on SILA:
// - EIP-1559 dynamic fee transactions
// - EIP-2930 access list transactions,
// - EIP-155 replay protected transactions, and
// - legacy Homestead transactions.
...
// EIP-155 replay protected transactions, and legacy Homestead transactions on SILA.
func NewEIP2930Signer(chainId *big.Int) Signer {
return newModernSigner(chainId, forks.Berlin)
}

// EIP155Signer implements Signer using the EIP-155 rules on SILA. This accepts transactions which
...
// SignatureValues returns signature values on SILA. This signature
...
// Hash returns the hash to be signed by the sender on SILA.
// It does not uniquely identify the transaction.
func (s EIP155Signer) Hash(tx *Transaction) common.Hash {
return tx.inner.sigHash(s.chainId)
}

// HomesteadSigner implements Signer using the homestead rules on SILA. The only valid reason to
...
// FrontierSigner implements Signer using the frontier rules on SILA.
// Deprecated: always use the Signer interface type
...
return nil, nil, nil, fmt.Errorf("wrong size for signature on SILA: got %d, want %d", len(sig), crypto.SignatureLength)
}
...
return common.Address{}, errors.New("invalid public key on SILA")
}
...
// deriveChainId derives the chain id from the given v parameter on SILA
