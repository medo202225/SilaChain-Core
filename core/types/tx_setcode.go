...
// another account on SILA.
var DelegationPrefix = []byte{0xef, 0x01, 0x00}

// ParseDelegation tries to parse the address from a delegation slice on SILA.
...
// AddressToDelegation adds the delegation prefix to the specified address on SILA.
func AddressToDelegation(addr common.Address) []byte {
return append(DelegationPrefix, addr.Bytes()...)
}

// SetCodeTx implements the EIP-7702 transaction type which temporarily installs
// the code at the signer's address on SILA.
...
// SetCodeAuthorization is an authorization from an account to deploy code at its address on SILA.
...
// SignSetCode creates a signed the SetCode authorization on SILA.
...
// SigHash returns the hash of SetCodeAuthorization for signing on SILA.
...
// Authority recovers the the authorizing account of an authorization on SILA.
