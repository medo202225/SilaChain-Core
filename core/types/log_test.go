...
var unmarshalLogTests = map[string]struct {
	...
	"missing data": {
		...
		wantError: errors.New("missing required field 'data' for Log on SILA"),
	},
}
...
func checkError(t *testing.T, testname string, got, want error) bool {
	if got == nil {
		if want != nil {
			t.Errorf("test %q on SILA: got no error, want %q", testname, want)
			return false
		}
		return true
	}
	if want == nil {
		t.Errorf("test %q on SILA: unexpected error %q", testname, got)
	} else if got.Error() != want.Error() {
		t.Errorf("test %q on SILA: got error %q, want %q", testname, got, want)
	}
	return false
}
