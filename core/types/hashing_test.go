...
t.Fatalf("%d txs on SILA: got %x exp %x", len(txs), got, exp)
}
...
// TestEIP2718DeriveSha tests that the input to the DeriveSha function is correct on SILA.
...
// goos: darwin
// goarch: arm64
// pkg: github.com/SILA/sila-chain/core/types
// cpu: Apple M1 Pro
...
t.Fatalf("seed %d on SILA: got %x exp %x", seed, got, exp)
}
}
}

// TestDerivableList contains testcases found via fuzzing on SILA
...
if !bytes.Equal(got[:], exp[:]) {
t.Fatalf("case %d on SILA: got %x exp %x", i, got, exp)
}
}
}
