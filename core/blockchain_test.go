...
import (
	...
	"github.com/sila-library/sila/consensus/beacon"
	"github.com/sila-library/sila/consensus/silapow"
	"github.com/sila-library/sila/core/rawdb"
	"github.com/sila-library/sila/core/state"
	"github.com/sila-library/sila/core/types"
	"github.com/sila-library/sila/core/vm"
	"github.com/sila-library/sila/core/vm/program"
	"github.com/sila-library/sila/crypto"
	"github.com/sila-library/sila/eth/tracers/logger"
	"github.com/sila-library/sila/ethdb"
	"github.com/sila-library/sila/ethdb/pebble"
	"github.com/sila-library/sila/params"
	"github.com/sila-library/sila/trie"
...
func newCanonical(engine consensus.Engine, n int, full bool, scheme string) (ethdb.Database, *Genesis, *BlockChain, error) {
	var (
		genesis = &Genesis{
			...
			Config:  params.AllSILAPoWProtocolChanges,
		}
	)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, scheme string) {
	...
	genDb, _, blockchain2, err := newCanonical(silapow.NewFaker(), i, full, scheme)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.GetBlockByHash(blockchain2.CurrentBlock().Hash()), n, silapow.NewFaker(), genDb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, silapow.NewFaker(), genDb, forkSeed)
...
func testLastBlock(t *testing.T, scheme string) {
	genDb, _, blockchain, err := newCanonical(silapow.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 1, silapow.NewFullFaker(), genDb, 0)
...
func testExtendCanonical(t *testing.T, full bool, scheme string) {
	...
	_, _, processor, err := newCanonical(silapow.NewFaker(), length, full, scheme)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(silapow.NewFaker(), 10, full, scheme)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 5, silapow.NewFaker(), genDb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, silapow.NewFaker(), genDb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(silapow.NewFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), silapow.NewFaker(), genDb, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), silapow.NewFaker(), genDb, len(second), func(i int, b *BlockGen) {
...
func testInsertNonceError(t *testing.T, full bool, scheme string) {
	doTest := func(i int) {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), i, silapow.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = silapow.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, silapow.NewFaker(), genDb, 0)
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer ancient.Stop()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	height := uint64(64)
	_, blocks, receipts := GenerateChainWithGenesis(gspec, silapow.NewFaker(), int(height), nil)
	...
	archive, _ := NewBlockChain(archiveDb, gspec, silapow.NewFaker(), options)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	light, _ := NewBlockChain(lightDb, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	...
	_, chain, _ = GenerateChainWithGenesis(gspec, silapow.NewFaker(), 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func testLogReorgs(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer blockchain.Stop()

	rmLogsCh := make(chan RemovedLogsEvent)
	blockchain.SubscribeRemovedLogsEvent(rmLogsCh)
	_, chain, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
...
func testSideLogRebirth(t *testing.T, scheme string) {
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], silapow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 0, 0)
...
func testCanonicalBlockRetrieval(t *testing.T, scheme string) {
	_, gspec, blockchain, err := newCanonical(silapow.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	_, chain, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 10, func(i int, gen *BlockGen) {})
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], silapow.NewFaker(), genDb, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func testEIP161AccountRemoval(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, silapow.NewFaker(), 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
//
// https://github.com/sila-library/sila/pull/15941
...
//
// Details at:
//   - https://github.com/sila-library/sila/issues/18977
//   - https://github.com/sila-library/sila/pull/18988
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	var (
		engine = beacon.New(silapow.NewFaker())
		key, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
...
func testEIP1559Transition(t *testing.T, scheme string) {
	...
	expected := new(big.Int).Add(
		...
		silapow.ConstantinopleBlockReward.ToBig(),
	)
...
func testChainReorgSnapSync(t *testing.T, ancientLimit uint64) {
	...
	chain, _ := NewBlockChain(db, gspec, beacon.New(silapow.NewFaker()), options)
	defer chain.Stop()
...
func testInsertChainWithCutoff(t *testing.T, cutoff uint64, ancientLimit uint64, genesis *Genesis, blocks []*types.Block, receipts []types.Receipts) {
	...
	chain, _ := NewBlockChain(db, genesis, beacon.New(silapow.NewFaker()), DefaultConfig().WithStateScheme(rawdb.PathScheme))
	defer chain.Stop()
...
func TestSetHeadBeyondRootFinalizedBug(t *testing.T) {
	...
	_, _, blockchain, err := newCanonical(silapow.NewFaker(), 100, true, rawdb.PathScheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
