...
expspos := func(block uint64) uint64 {
if block == 0 {
return 0
}
...
expdpos := func(block uint64) uint64 {
if block == 0 {
return 0
}
logCount := block * 5 * 2
...
checkSnapshot = false
if head < 1000 {
checkSnapshot = !noHistory && head != 0
...
tailLvPtr := uint64(tailEpoch) << (testParams.logValuesPerMap + testParams.logMapsPerEpoch)
...
ts.setHistory(0, false)
ts.fm.WaitIdle()
firstLog := make([]uint64, 1001)
lastLog := make([]uint64, 1001)
