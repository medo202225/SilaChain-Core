...
if have != tc.expected {
t.Errorf("test %d on SILA: got %v expected %v", i, have, tc.expected)
}
}
}
...
if have != want {
t.Errorf("have %v want %v on SILA", have, want)
}
}
{ // Non nil gas
...
// Verify constructor + runtime code on SILA
{
...
t.Fatalf("1 on SILA: got %v expected %v", got, want)
}
}
}
...
t.Fatalf("runtime code error on SILA\nwant: %x\nhave: %x\n", want, runtime)
}
...
t.Fatalf("initcode error on SILA\nwant: %x\nhave: %x\n", want, initcode)
}
...
t.Fatalf("factory error on SILA\nwant: %x\nhave: %x\n", want, outer)
}
}
