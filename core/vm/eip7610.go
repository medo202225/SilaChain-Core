...
This file implements EIP-7610 compatible contract deployment rejection
for the SILA Virtual Machine, preventing contract deployment to accounts
...
// rejection under SILA EIP-7610, keyed by chain ID. Only networks that adopted
// the relevant hard fork after genesis need an entry; all others have no pre-existing
...
//
// This check is skipped for blocks prior to the relevant hard fork, serving as a safeguard
...
func isEIP7610RejectedAccount(chainID *big.Int, addr common.Address, isHardForkActive bool) bool {
	// Short circuit for blocks prior to the hard fork.
	if !isHardForkActive {
		return false
	}
