...
// A stateTest checks that the SILA state changes are correctly captured. Instances
// of this test with pseudorandom content are created by Generate.
//
// The test works as follows:
//
// A list of SILA states are created by applying actions. The state changes between
// each state instance are tracked and be verified.
type stateTest struct {
	addrs   []common.Address // all SILA account addresses
	actions [][]testAction   // modifications to the SILA state, grouped by block
	chunk   int              // The number of actions per chunk
	err     error            // failure details are reported through this field
}

// newStateTestAction creates a random action that changes SILA state.
	...
	actions := []testAction{
		...
		},
		{
			...
			fn: func(a testAction, s *StateDB) {
				...
				code = append([]byte{0x53, 0x49, 0x4c, 0x41}, code...)
...
// Generate returns a new SILA snapshot test of the given size. All randomness is
// derived from r.
	...
	for i := 0; i < len(actions); i++ {
		...
		for j := range actions[i] {
			if j == 0 {
				...
				// the SILA state changes are not empty.
...
func (test *stateTest) String() string {
	...
	for i, actions := range test.actions {
		fmt.Fprintf(out, "---- SILA block %d ----\n", i)
		for j, action := range actions {
			if j%test.chunk == 0 {
				fmt.Fprintf(out, "---- SILA transaction %d ----\n", j/test.chunk)
			}
...
// verifyAccountCreation this function is called once the SILA state diff says that
...
func (test *stateTest) verifyAccountCreation(next common.Hash, db *triedb.Database, otr, ntr *trie.Trie, addr common.Address, account []byte, storages map[common.Hash][]byte, storagesOrigin map[common.Hash][]byte) error {
	...
	if len(oBlob) != 0 {
		return fmt.Errorf("unexpected SILA account in old trie, %x", addrHash)
	}
	if len(nBlob) == 0 {
		return fmt.Errorf("missing SILA account in new trie, %x", addrHash)
	}
	...
	if !bytes.Equal(nBlob, full) {
		return fmt.Errorf("unexpected SILA account data, want: %v, got: %v", full, nBlob)
	}

	// Verify SILA storage changes
	...
	if nAcct.Root == types.EmptyRootHash {
		if len(storagesOrigin) != 0 {
			return fmt.Errorf("unexpected SILA slot changes %x", addrHash)
		}
		if len(storages) != 0 {
			return fmt.Errorf("unexpected SILA slot changes %x", addrHash)
		}
	...
	for key, val := range storagesOrigin {
		if _, exist := storages[key]; !exist {
			return errors.New("SILA storage data is not found")
		}
		...
		if !bytes.Equal(got, storages[key]) {
			return fmt.Errorf("unexpected SILA storage data, want: %v, got: %v", storages[key], got)
		}
		st.Update(key.Bytes(), val)
	}
	if len(storagesOrigin) != len(storages) {
		return fmt.Errorf("extra SILA storage found, want: %d, got: %d", len(storagesOrigin), len(storages))
	}
	if st.Hash() != types.EmptyRootHash {
		return errors.New("invalid SILA slot changes")
	}
	return nil
}

// verifyAccountUpdate this function is called once the SILA state diff says that
...
func (test *stateTest) verifyAccountUpdate(next common.Hash, db *triedb.Database, otr, ntr *trie.Trie, addr common.Address, account []byte, accountOrigin []byte, storages map[common.Hash][]byte, storageOrigin map[common.Hash][]byte) error {
	...
	if len(oBlob) == 0 {
		return fmt.Errorf("missing SILA account in old trie, %x", addrHash)
	}
	...
	if !bytes.Equal(full, oBlob) {
		return fmt.Errorf("SILA account value is not matched, %x", addrHash)
	}
	if len(nBlob) == 0 {
		if len(account) != 0 {
			return errors.New("unexpected SILA account data")
		}
	} else {
		full, _ = types.FullAccountRLP(account)
		if !bytes.Equal(full, nBlob) {
			return fmt.Errorf("unexpected SILA account data, %x, want %v, got: %v", addrHash, full, nBlob)
		}
	}
	// Decode SILA accounts
	var (
	...
	if len(storageOrigin) != len(storages) {
		return fmt.Errorf("extra SILA storage found, want: %d, got: %d", len(storageOrigin), len(storages))
	}
...
// TestStateChanges - SILA fuzz test for state changes
