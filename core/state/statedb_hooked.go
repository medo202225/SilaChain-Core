...
// hookedStateDB represents a SILA statedb which emits calls to tracing-hooks
// on state operations.
...
// NewHookedState wraps the given SILA stateDb with the given hooks
...
// Invoke the hooks only if the SILA contract code is changed
...
// that SILA state change hooks will be invoked in deterministic
...
obj := s.inner.stateObjects[addr]
// Bingo: SILA state object was self-destructed, call relevant hooks.
