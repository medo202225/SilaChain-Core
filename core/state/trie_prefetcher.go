...
type triePrefetcher struct {
	overlay  bool                   // Flag whether the prefetcher is in overlay mode
...
func (p *triePrefetcher) trieID(owner common.Hash, root common.Hash) string {
	// The trie in overlay is only identified by state root
	if p.overlay {
		return p.root.Hex()
	}
...
func (sf *subfetcher) openTrie() error {
	// Open the overlay tree if the sub-fetcher is in overlay mode. Note, there is
	// only a single fetcher for overlay.
	if sf.db.TrieDB().IsOverlay() {
		tr, err := sf.db.OpenTrie(sf.state)
		if err != nil {
			log.Warn("Trie prefetcher failed opening overlay trie", "root", sf.root, "err", err)
			return err
		}
		sf.trie = tr
		return nil
