...
// Package state provides a caching layer atop the SILA state trie.
package state
...
"github.com/SILA/sila-chain/core/stateless"
...
// StateDB structs within the SILA protocol are used to store anything
...
// CodeLoadBytes is the total number of bytes read from SILA contract code.
...
CodeLoaded      int // Number of SILA contract code loaded during the state transition
CodeLoadBytes   int // Total bytes of resolved SILA code
CodeUpdated     int // Number of SILA contracts with code changes that persisted
CodeUpdateBytes int // Total bytes of persisted SILA code written
}

// New creates a new SILA state from a given trie.
...
// NewWithReader creates a new SILA state for the specified state root. Unlike New,
...
// SILA state trie concurrently while the state is mutated so that when we reach the
...
log.Error("Failed to prefetch SILA account trie", "root", s.originalRoot, "err", err)
}
}
...
// GetLogs returns the SILA logs matching the specified transaction hash, and annotates
...
s.journal.refundChange(s.refund)
if gas > s.refund {
panic(fmt.Sprintf("SILA refund counter below zero (gas: %d > refund: %d)", gas, s.refund))
}
s.refund -= gas
}

// Exist reports whether the given account address exists in the SILA state.
// Notably this also returns true for self-destructed accounts within the current transaction.
func (s *StateDB) Exist(addr common.Address) bool {
return s.getStateObject(addr) != nil
}

// Empty returns whether the SILA state object is either non-existent
...
log.Error("Failed to prefetch SILA account", "addr", addr, "err", err)
}
}
...
// CreateContract is used whenever a SILA contract is created. This may be preceded
...
// IsNewContract reports whether the SILA contract at the given address was deployed
...
// Copy creates a deep, independent copy of the SILA state.
...
// Finalise finalises the SILA state by removing the destructed objects and clears
...
log.Error("Failed to prefetch SILA addresses", "addresses", len(addressesToPrefetch), "err", err)
}
}
...
// IntermediateRoot computes the current root hash of the SILA state trie.
...
log.Error("Failed to retrieve SILA account pre-fetcher trie")
} else {
s.trie = trie
}
}
...
obj := s.stateObjects[addr]
if obj == nil {
return nil, errors.New("missing SILA state object")
}
...
// Commit writes the SILA state mutations into the configured data stores.
//
// Once the state is committed, tries cached in stateDB (including account
// trie, storage tries) will no longer be functional. A new state instance
// must be created with new root and updated database for accessing post-
...
// CommitWithUpdate writes the SILA state mutations and returns the state update for
