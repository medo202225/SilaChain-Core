...
"github.com/SILA/sila-chain/triedb/hashdb"
...
fn: func(a testAction, s *StateDB) {
s.AddSlotToAccessList(addr, common.Hash{byte(a.args[0])})
},
args: make([]int64, 1),
},
{
...
return fmt.Errorf("dirty storage err, have\n%v\nwant\n%v", print(obj.dirtyStorage), print(other.dirtyStorage))
}
}
{
...
if !checkstate.accessList.Equal(state.accessList) {
return fmt.Errorf("AccessLists are wrong, have \n%v\nwant\n%v", checkstate.accessList.PrettyPrint(), state.accessList.PrettyPrint())
}
if state.GetRefund() != checkstate.GetRefund() {
return fmt.Errorf("got GetRefund() == %d, want GetRefund() == %d", state.GetRefund(), checkstate.GetRefund())
}
if !reflect.DeepEqual(state.GetLogs(common.Hash{}, 0, common.Hash{}, 0), checkstate.GetLogs(common.Hash{}, 0, common.Hash{}, 0)) {
return fmt.Errorf("got GetLogs(common.Hash{}) == %v, want GetLogs(common.Hash{}) == %v", state.GetLogs(common.Hash{}, 0, common.Hash{}, 0), checkstate.GetLogs(common.Hash{}, 0, common.Hash{}, 0))
}
...
state.SetBalance(addr, uint256.NewInt(42), tracing.BalanceChangeUnspecified)
state.SetCode(addr, []byte("hello"), tracing.CodeChangeUnspecified)
state.SetState(addr, skey1, sval1)
