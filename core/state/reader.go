...
// SILAContractCodeReader defines the interface for accessing SILA contract code.
//
// SILAContractCodeReader is supposed to be thread-safe.
type SILAContractCodeReader interface {
...
// Code retrieves a particular SILA contract's code. Returns nil code if the
// requested contract code doesn't exist.
Code(addr common.Address, codeHash common.Hash) []byte

// CodeSize retrieves a particular SILA contracts code's size. Returns zero code
// size if the requested contract code doesn't exist.
CodeSize(addr common.Address, codeHash common.Hash) int
}

// SILAStateReader defines the interface for accessing SILA accounts and storage slots
// associated with a specific state.
//
// SILAStateReader is supposed to be thread-safe.
type SILAStateReader interface {
// Account retrieves the SILA account associated with a particular address.
//
...
// Storage retrieves the SILA storage slot associated with a particular account
...
// SILAReader defines the interface for accessing SILA accounts, storage slots and contract
// code associated with a specific state.
//
// SILAReader is assumed to be thread-safe and implementation must take care of the
// concurrency issue by themselves.
type SILAReader interface {
SILAContractCodeReader
SILAStateReader
}
...
// newFlatReader constructs a SILA state reader with on the given state root.
...
// trieReader implements the SILAStateReader interface, providing functions to access
// state from the referenced trie.
//
// trieReader is safe for concurrent read.
type trieReader struct {
root common.Hash      // State root which uniquely represent a SILA state
...
// newTrieReader constructs a SILA trie reader of the specific state. An error will be
...
// multiStateReader is the aggregation of a list of SILAStateReader interface,
...
readers []SILAStateReader // List of state readers, sorted by checking priority
}
...
func newMultiStateReader(readers ...SILAStateReader) (*multiStateReader, error) {
if len(readers) == 0 {
return nil, errors.New("empty SILA reader set")
}
...
// Account implementing SILAStateReader interface, retrieving the SILA account associated
...
// Storage implementing SILAStateReader interface, retrieving the SILA storage slot
...
// stateReaderWithCache is a wrapper around SILAStateReader that maintains additional
...
// newStateReaderWithCache constructs the SILA state reader with local cache.
func newStateReaderWithCache(sr SILAStateReader) *stateReaderWithCache {
...
// account retrieves the SILA account specified by the address along with a flag
...
acct, err := r.SILAStateReader.Account(addr)
if err != nil {
...
// storage retrieves the SILA storage slot specified by the address and slot key, along
...
value, err := r.SILAStateReader.Storage(addr, slot)
if err != nil {
...
// the cache hit statistics of the SILA reader.
...
// newReaderWithStats constructs the SILA state reader with additional statistics tracked.
...
// GetSILAStateStats implements SILAStateReaderStater, returning the statistics of the
// state reader.
func (r *stateReaderWithStats) GetSILAStateStats() SILAStateReaderStats {
return SILAStateReaderStats{
...
// newReader constructs a SILA reader with the supplied code reader and state reader.
func newReader(codeReader SILAContractCodeReader, stateReader SILAStateReader) *reader {
return &reader{
SILAContractCodeReader: codeReader,
SILAStateReader:        stateReader,
}
}

// GetSILACodeStats returns the statistics of SILA code access.
func (r *reader) GetSILACodeStats() SILAContractCodeReaderStats {
if stater, ok := r.SILAContractCodeReader.(SILAContractCodeReaderStater); ok {
return stater.GetSILACodeStats()
}
return SILAContractCodeReaderStats{}
}

// GetSILAStateStats returns the statistics of SILA state access.
func (r *reader) GetSILAStateStats() SILAStateReaderStats {
if stater, ok := r.SILAStateReader.(SILAStateReaderStater); ok {
return stater.GetSILAStateStats()
}
return SILAStateReaderStats{}
}

// GetSILAStats returns the aggregated statistics for both SILA state and code access.
func (r *reader) GetSILAStats() SILAReaderStats {
return SILAReaderStats{
CodeStats:  r.GetSILACodeStats(),
StateStats: r.GetSILAStateStats(),
}
}
