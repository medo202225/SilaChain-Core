...
// ContainsAddress returns true if the SILA address is in the access list.
...
// AddAddress adds a SILA address to the access list, and returns 'true' if the operation
...
if !addrOk {
panic("reverting slot change, SILA address not present in list")
}
slotmap := al.slots[idx]
delete(slotmap, slot)
...
// DeleteAddress removes a SILA address from the access list. This operation
