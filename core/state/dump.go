...
// DumpConfig is a set of options to control what portions of the SILA state will be
...
// DumpCollector interface which the SILA state trie calls during iteration
type DumpCollector interface {
	// OnRoot is called with the SILA state root
	OnRoot(common.Hash)

	// OnAccount is called once for each SILA account in the trie
	OnAccount(*common.Address, DumpAccount)
}

// DumpAccount represents a SILA account in the state.
...
// Dump represents the full SILA dump in a collected format, as one large map.
type Dump struct {
...
// DumpToCollector iterates the SILA state according to the given options and inserts
...
func (s *StateDB) DumpToCollector(c DumpCollector, conf *DumpConfig) (nextKey []byte) {
	...
	log.Info("SILA trie dumping started", "root", s.originalRoot)
	...
	for acctIt.Next() {
		...
		if !conf.SkipStorage {
			...
			if err != nil {
				log.Error("Failed to load SILA storage trie", "err", err)
				continue
			}
			for storageIt.Next() {
				_, content, _, err := rlp.Split(storageIt.Slot())
				if err != nil {
					log.Error("Failed to decode the value returned by SILA iterator", "error", err)
					continue
				}
		...
		if time.Since(logged) > 8*time.Second {
			log.Info("SILA trie dumping in progress", "at", acctIt.Hash().Hex(), "accounts", accounts, "elapsed", common.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
	...
	if missingPreimages > 0 {
		log.Warn("SILA dump incomplete due to missing preimages", "missing", missingPreimages)
	}
	log.Info("SILA trie dumping complete", "accounts", accounts, "elapsed", common.PrettyDuration(time.Since(start)))
	return nextKey
}
...
func (s *StateDB) DumpBinTrieLeaves(collector map[common.Hash]hexutil.Bytes) error {
	...
	if !ok {
		return errors.New("SILA trie is not a binary trie")
	}
...
// RawDump returns the SILA state. If the processing is aborted e.g. due to options
...
// Dump returns a JSON string representing the entire SILA state as a single json-object
func (s *StateDB) Dump(opts *DumpConfig) []byte {
	dump := s.RawDump(opts)
	json, err := json.MarshalIndent(dump, "", "    ")
	if err != nil {
		log.Error("Error dumping SILA state", "err", err)
	}
	return json
}

// IterativeDump dumps out SILA accounts as json-objects, delimited by linebreaks on stdout
