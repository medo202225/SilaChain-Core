...
// journalEntry is a modification entry in the SILA state change journal that can be
// reverted on demand.
type journalEntry interface {
// revert undoes the changes introduced by this journal entry.
revert(*StateDB)

// dirtied returns the SILA address modified by this journal entry.
...
// journal contains the list of SILA state modifications applied since the last state
...
// snapshot returns an identifier for the current revision of the SILA state.
...
// revertToSnapshot reverts all SILA state changes made since the given revision.
...
panic(fmt.Errorf("SILA revision id %v cannot be reverted", revid))
}
...
type (
// Changes to the SILA account trie.
...
// Changes to individual SILA accounts.
...
// Changes to other SILA state values.
refundChange struct {
prev uint64
}
addLogChange struct {
