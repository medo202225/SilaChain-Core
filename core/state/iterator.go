...
// nodeIterator is an iterator to traverse the entire SILA state trie post-order,
...
type nodeIterator struct {
	state *StateDB // SILA state being iterated
	tr    Trie     // Primary account trie for traversal

	stateIt trie.NodeIterator // Primary iterator for the global SILA state trie
	dataIt  trie.NodeIterator // Secondary iterator for the data trie of a contract

	accountHash common.Hash // Hash of the node containing the account
	codeHash    common.Hash // Hash of the SILA contract source code
	code        []byte      // Source code associated with a SILA contract
...
// newNodeIterator creates a post-order SILA state node iterator.
...
// step moves the iterator to the next entry of the SILA state trie.
func (it *nodeIterator) step() error {
	...
	// Step to the next SILA state trie node, terminating if we're out of nodes
	...
	// If the SILA state trie node is an internal entry, leave as is
	if !it.stateIt.Leaf() {
		return nil
	}
	...
	if preimage == nil {
		return errors.New("SILA account address is not available")
	}
	address := common.BytesToAddress(preimage)

	// Traverse the storage slots belong to the SILA account
	...
	if !bytes.Equal(account.CodeHash, types.EmptyCodeHash.Bytes()) {
		...
		if len(it.code) == 0 {
			return fmt.Errorf("SILA code is not found: %x", account.CodeHash)
		}
	}
...
// retrieve pulls and caches the current SILA state entry the iterator is traversing.
