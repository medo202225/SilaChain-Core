...
// differentiate if a forkid.next field is a block number or a timestamp.
...
// NewID calculates the SILA fork ID from the chain config, genesis hash, head and time.
...
// NewIDWithChain calculates the SILA fork ID from an existing chain instance.
...
// If our head is beyond this fork, continue to the next
if head >= fork {
continue
}
