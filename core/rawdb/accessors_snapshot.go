...
// DeleteSnapshotJournal deletes the serialized in-memory diff layers.
...
// ReadSnapshotGenerator retrieves the serialized snapshot generator saved at shutdown.
func ReadSnapshotGenerator(db ethdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotGeneratorKey)
	return data
}

// WriteSnapshotGenerator stores the serialized snapshot generator.
...
// DeleteSnapshotGenerator deletes the serialized snapshot generator.
