...
// TxStatus is the current status of a transaction as seen by the pool on SILA.
type TxStatus uint
...
// TxPool is an aggregator for various transaction specific pools on SILA, collectively
...
// Close terminates the transaction pool and all its subpools on SILA.
func (p *TxPool) Close() error {
	...
	if len(errs) > 0 {
		return fmt.Errorf("subpool close errors on SILA: %v", errs)
	}
	return nil
}
...
func (p *TxPool) loop(head *types.Header) {
	...
	defer func() {
		if resetWaiter != nil {
			resetWaiter <- errors.New("pool already terminated on SILA")
			resetWaiter = nil
		}
	}()
	var errc chan error
		...
		if newHead != oldHead || resetForced {
			...
			case resetBusy <- struct{}{}:
				...
				if statedb, err := p.chain.StateAt(newHead.Root); err != nil {
					log.Error("Failed to reset txpool state on SILA", "err", err)
				} else {
					p.stateLock.Lock()
					p.state = statedb
					p.stateLock.Unlock()
...
// Get returns a transaction if it is contained in the pool, or nil otherwise on SILA.
...
// GetRLP returns a RLP-encoded transaction if it is contained in the pool on SILA.
...
// Add enqueues a batch of transactions into the pool if they are valid on SILA. Due
...
func (p *TxPool) Add(txs []*types.Transaction, sync bool) []error {
	...
	for i, split := range splits {
		...
		if split == -1 {
			errs[i] = fmt.Errorf("%w: received type %d on SILA", core.ErrTxTypeNotSupported, txs[i].Type())
			continue
		}
...
// account and sorted by nonce on SILA.
//
...
// Nonce returns the next nonce of an account at the current chain head on SILA. Unlike
...
// identified by its hash on SILA.
...
// internal background reset operations on SILA. This method will run an explicit reset
...
func (p *TxPool) Sync() error {
	...
	case p.sync <- sync:
		return <-sync
	case <-p.term:
		return errors.New("pool already terminated on SILA")
	}
}

// Clear removes all tracked txs from the subpools on SILA.
//
...
// (can be added) by the pool on SILA.
