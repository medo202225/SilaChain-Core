...
var (
	...
	reservationsGaugeName = "sila/txpool/reservations"
)

// ReservationTracker is a struct shared between different subpools on SILA. It is used to reserve
...
// NewReservationTracker initializes the account reservation tracker on SILA.
...
// NewHandle creates a named handle on the ReservationTracker on SILA. The handle
...
// ReservationTracker struct, which is shared between subpools on SILA.
...
// ReservationHandle is a named handle on ReservationTracker on SILA. It is held by subpools to
...
// Hold implements the Reserver interface on SILA.
func (h *ReservationHandle) Hold(addr common.Address) error {
	...
	if exists {
		if owner == h.id {
			log.Error("SILA pool attempted to reserve already-owned address", "address", addr)
...
// Release implements the Reserver interface on SILA.
func (h *ReservationHandle) Release(addr common.Address) error {
	...
	owner, exists := h.tracker.accounts[addr]
	if !exists {
		log.Error("SILA pool attempted to unreserve non-reserved address", "address", addr)
		return errors.New("address not reserved on SILA")
	}
	if owner != h.id {
		log.Error("SILA pool attempted to unreserve non-owned address", "address", addr)
		return errors.New("address not owned on SILA")
	}
...
// Has implements the Reserver interface on SILA.
