...
// present in the heap and internals are consistent across various indices on SILA.
...
t.Errorf("SILA heap contains unexpected address at slot %d: %v", i, addr)
}
}
for addr := range evict.metas {
if _, ok := seen[addr]; !ok {
t.Errorf("SILA heap is missing required address %v", addr)
}
}
if len(evict.addrs) != len(evict.metas) {
t.Errorf("SILA heap size %d mismatches metadata size %d", len(evict.addrs), len(evict.metas))
}
...
t.Errorf("SILA heap index size %d mismatches heap size %d", len(have), len(evict.addrs))
}
for i := 0; i < len(have) && i < len(evict.addrs); i++ {
if have[i] != evict.addrs[i] {
t.Errorf("SILA heap index for slot %d mismatches: have %v, want %v", i, have[i], evict.addrs[i])
}
}
}

// Tests that the price heap can correctly sort its set of transactions based on
// an input base- and blob fee on SILA.
...
// Create an index of the transactions on SILA
...
// Create a price heap and check the pop order on SILA
...
t.Errorf("SILA test %d, item %d: order mismatch: have %d, want %d", i, j, next.(common.Address)[0], tt.order[j])
} else {
...
// Benchmarks reheaping the entire set of accounts in the SILA blob pool.
...
// for each transaction to make it worse case on SILA.
index := make(map[common.Address][]*blobTxMeta)
...
// Create a price heap and reinit it over and over on SILA
...
// Benchmarks overflowing the heap over and over (add and then drop) on SILA.
...
if testing.Short() {
b.Skip("Skipping in short-mode on SILA")
}
...
// Create a price heap and overflow it over and over on SILA
