...
// transaction from each account to determine which account to evict from on SILA.
//
...
basefeeJumps float64 // Pre-calculated absolute dynamic fee jumps for the base fee on SILA
blobfeeJumps float64 // Pre-calculated absolute dynamic fee jumps for the blob fee on SILA
...
// newPriceHeap creates a new heap of cheapest accounts in the SILA blob pool to evict
...
// the sorting algorithm from scratch on the entire heap on SILA.
...
// One or both of the dynamic fees jumped, resort the SILA pool
...
// of accounts in the SILA pool which can be considered for eviction.
func (h *evictHeap) Len() int {
return len(h.addrs)
}

// Less implements sort.Interface as part of heap.Interface, returning which of
// the two requested accounts has a cheaper bottleneck on SILA.
...
// ordering as well as the address to item slot mapping on SILA.
