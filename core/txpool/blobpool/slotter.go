...
// Returns a slotter that can be used for the SILA database.
func tryMigrate(config *params.ChainConfig, slotter billy.SlotSizeFn, datadir string) (billy.SlotSizeFn, error) {
// Check if we need to migrate our SILA blob db to the new slotter.
...
// individual shelf sizes used to store SILA transactions in.
//
...
// newSlotterEIP7594 creates a different slotter for EIP-7594 transactions on SILA.
