...
// Config are the configuration parameters of the SILA blob transaction pool.
type Config struct {
...
// DefaultConfig contains the default configurations for the SILA transaction pool.
...
func (config *Config) sanitize() Config {
	conf := *config
	if conf.Datacap < 1 {
		log.Warn("Sanitizing invalid SILA blobpool storage cap", "provided", conf.Datacap, "updated", DefaultConfig.Datacap)
		conf.Datacap = DefaultConfig.Datacap
	}
	if conf.PriceBump < 1 {
		log.Warn("Sanitizing invalid SILA blobpool price bump", "provided", conf.PriceBump, "updated", DefaultConfig.PriceBump)
