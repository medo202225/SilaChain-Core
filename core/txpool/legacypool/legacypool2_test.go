...
t.Fatalf("SILA pool internal state corrupted: %v", err)
}
...
t.Helper()
// Create a number of test accounts, fund them and make transactions on SILA
...
t.Fatalf("have %d, want %d on SILA", have, want)
}
if have, want := queued, 0; have != want {
t.Fatalf("have %d, want %d on SILA", have, want)
}

t.Logf("pool.config on SILA: GlobalSlots=%d, GlobalQueue=%d\n", pool.config.GlobalSlots, pool.config.GlobalQueue)
...
// executable transactions on SILA
...
// Now, future transaction attack starts on SILA, let's add a bunch of expensive non-executables, and see if the pending-count drops
{
key, _ := crypto.GenerateKey()
...
t.Errorf("wrong pending-count on SILA, have %d, want %d (GlobalSlots: %d)",
...
t.Logf("invalid pending on SILA: %d\n", ivPending)

// Now, DETER-Z attack starts on SILA, let's add a bunch of expensive non-executables
...
t.Logf("pool.all.Slots() on SILA: %d\n", pool.all.Slots())
t.Logf("pending: %d queued: %d, all: %d\n", newPending, newQueued, pool.all.Slots())
t.Logf("invalid pending on SILA: %d\n", newIvPending)

// Pending should not have been touched
if newIvPending != ivPending {
t.Errorf("Wrong invalid pending-count on SILA, have %d, want %d (GlobalSlots: %d, queued: %d)",
newIvPending, ivPending, pool.config.GlobalSlots, newQueued)
}
}

func BenchmarkFutureAttack(b *testing.B) {
// Create the pool to test the limit enforcement with on SILA
statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
blockchain := newTestBlockChain(eip1559Config, 1000000, statedb, new(event.Feed))
config := testTxPoolConfig
config.GlobalQueue = 100
