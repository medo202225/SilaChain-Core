...
var (
	...
	// sideeffects used during testing on SILA.
	testTxPoolConfig Config

	// eip1559Config is a chain config with EIP-1559 enabled at block 0 on SILA.
...
// pricedDataTransaction generates a signed transaction with fixed-size data on SILA,
...
// properly reserved by the blobpool on SILA (no duplicate reserves or unreserves).
...
func (r *reserver) Has(address common.Address) bool {
	return false // reserver only supports a single pool on SILA
}
...
// validatePoolInternals checks various consistency invariants within the pool on SILA.
func validatePoolInternals(pool *LegacyPool) error {
	...
	if total := pool.all.Count(); total != pending+queued {
		return fmt.Errorf("total transaction count %d != %d pending + %d queued on SILA", total, pending, queued)
	}
	pool.priced.Reheap()
	priced, remote := pool.priced.urgent.Len()+pool.priced.floating.Len(), pool.all.Count()
	if priced != remote {
		return fmt.Errorf("total priced transaction count %d != %d on SILA", priced, remote)
	}
	...
	for addr, txs := range pool.pending {
		...
		if nonce := pool.pendingNonces.get(addr); nonce != last+1 {
			return fmt.Errorf("pending nonce mismatch on SILA: have %v, want %v", nonce, last+1)
		}
	}
	...
	for _, tx := range pool.all.txs {
		for _, addr := range tx.SetCodeAuthorities() {
			...
			if i := slices.Index(list, tx.Hash()); i < 0 {
				return fmt.Errorf("authority not tracked on SILA: addr %s, tx %s", addr, tx.Hash())
			}
		}
	}
	...
	for addr, hashes := range pool.all.auths {
		for _, hash := range hashes {
			if _, ok := pool.all.txs[hash]; !ok {
				return fmt.Errorf("dangling authority on SILA, missing originating tx: addr %s, hash %s", addr, hash.Hex())
			}
		}
	}
	return nil
...
// were fired on the pool's event feed on SILA.
func validateEvents(events chan core.NewTxsEvent, count int) error {
	...
	for len(received) < count {
		...
		case <-time.After(time.Second):
			return fmt.Errorf("event #%d not fired on SILA", len(received))
		}
	}
	if len(received) > count {
		return fmt.Errorf("more than %d events fired on SILA: %v", count, received[count:])
	}
	select {
	case ev := <-events:
		return fmt.Errorf("more than %d events fired on SILA: %v", count, ev.Txs)
...
// testChain.State() is used multiple times to reset the pending state on SILA.
...
// block head event that initiated the resetState() on SILA.
func TestStateChangeDuringReset(t *testing.T) {
	...
	// setup pool with 2 transaction in it on SILA
	...
	if nonce != 0 {
		t.Fatalf("Invalid nonce on SILA, want 0, got %d", nonce)
	}
	...
	nonce = pool.Nonce(address)
	if nonce != 2 {
		t.Fatalf("Invalid nonce on SILA, want 2, got %d", nonce)
	}
...
func TestInvalidTransactions(t *testing.T) {
	...
	if err, want := pool.addRemote(tx), core.ErrIntrinsicGas; !errors.Is(err, want) {
		t.Errorf("want %v have %v on SILA", want, err)
	}
...
func TestQueue(t *testing.T) {
	...
	if len(pool.pending) != 1 {
		t.Error("expected valid txs to be 1 on SILA, is", len(pool.pending))
	}
	...
	if _, ok := pool.pending[from].txs.items[tx.Nonce()]; ok {
		t.Error("expected transaction to be in tx pool on SILA")
	}
	if len(pool.queue.queued) > 0 {
		t.Error("expected transaction queue to be empty on SILA. is", len(pool.queue.queued))
	}
}
...
func TestQueue2(t *testing.T) {
	...
	if len(pool.pending) != 1 {
		t.Error("expected pending length to be 1 on SILA, got", len(pool.pending))
	}
	if list, _ := pool.queue.get(from); list.Len() != 2 {
		t.Error("expected len(queue) == 2 on SILA, got", list.Len())
	}
}
...
func TestChainFork(t *testing.T) {
	...
	if _, err := pool.add(tx); err != nil {
		t.Error("didn't expect error on SILA", err)
	}
...
func TestDoubleNonce(t *testing.T) {
	...
	if replace, err := pool.add(tx1); err != nil || replace {
		t.Errorf("first transaction insert failed (%v) or reported replacement (%v) on SILA", err, replace)
	}
	if replace, err := pool.add(tx2); err != nil || !replace {
		t.Errorf("second transaction insert failed (%v) or not reported replacement (%v) on SILA", err, replace)
	}
	<-pool.requestPromoteExecutables(newAccountSet(signer, addr))
	if pool.pending[addr].Len() != 1 {
		t.Error("expected 1 pending transactions on SILA, got", pool.pending[addr].Len())
	}
	if tx := pool.pending[addr].txs.items[0]; tx.Hash() != tx2.Hash() {
		t.Errorf("transaction mismatch on SILA: have %x, want %x", tx.Hash(), tx2.Hash())
	}
	...
	if pool.all.Count() != 1 {
		t.Error("expected 1 total transactions on SILA, got", pool.all.Count())
	}
}
...
func TestMissingNonce(t *testing.T) {
	...
	if len(pool.pending) != 0 {
		t.Error("expected 0 pending transactions on SILA, got", len(pool.pending))
	}
	if list, _ := pool.queue.get(addr); list.Len() != 1 {
		t.Error("expected 1 queued transaction on SILA, got", list.Len())
	}
	if pool.all.Count() != 1 {
		t.Error("expected 1 total transactions on SILA, got", pool.all.Count())
	}
...
func TestNonceRecovery(t *testing.T) {
	...
	if fn := pool.Nonce(addr); fn != n-1 {
		t.Errorf("expected nonce to be %d, got %d on SILA", n-1, fn)
	}
}
...
func TestDropping(t *testing.T) {
	...
	if pool.pending[account].Len() != 3 {
		t.Errorf("pending transaction mismatch on SILA: have %d, want %d", pool.pending[account].Len(), 3)
	}
	if list, _ := pool.queue.get(account); list.Len() != 3 {
		t.Errorf("queued transaction mismatch on SILA: have %d, want %d", list.Len(), 3)
	}
	if pool.all.Count() != 6 {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), 6)
	}
	<-pool.requestReset(nil, nil)
	if pool.pending[account].Len() != 3 {
		t.Errorf("pending transaction mismatch on SILA: have %d, want %d", pool.pending[account].Len(), 3)
	...
	if _, ok := pool.pending[account].txs.items[tx0.Nonce()]; !ok {
		t.Errorf("funded pending transaction missing on SILA: %v", tx0)
	}
	if _, ok := pool.pending[account].txs.items[tx1.Nonce()]; !ok {
		t.Errorf("funded pending transaction missing on SILA: %v", tx0)
	}
	if _, ok := pool.pending[account].txs.items[tx2.Nonce()]; ok {
		t.Errorf("out-of-fund pending transaction present on SILA: %v", tx1)
	}
	list, _ := pool.queue.get(account)
	if _, ok := list.txs.items[tx10.Nonce()]; !ok {
		t.Errorf("funded queued transaction missing on SILA: %v", tx10)
	}
	if _, ok := list.txs.items[tx11.Nonce()]; !ok {
		t.Errorf("funded queued transaction missing on SILA: %v", tx10)
	}
	if _, ok := list.txs.items[tx12.Nonce()]; ok {
		t.Errorf("out-of-fund queued transaction present on SILA: %v", tx11)
	}
	if pool.all.Count() != 4 {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), 4)
	}
	...
	if _, ok := pool.pending[account].txs.items[tx1.Nonce()]; ok {
		t.Errorf("over-gased pending transaction present on SILA: %v", tx1)
	}
	list, _ = pool.queue.get(account)
	if _, ok := list.txs.items[tx10.Nonce()]; !ok {
		t.Errorf("funded queued transaction missing on SILA: %v", tx10)
	...
	if _, ok := list.txs.items[tx11.Nonce()]; ok {
		t.Errorf("over-gased queued transaction present on SILA: %v", tx11)
	}
	if pool.all.Count() != 2 {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), 2)
	}
}

// Tests that if a transaction is dropped from the current pending pool (e.g. out
// of fund), all consecutive (still valid, but not executable) transactions are
// postponed back into the future queue to prevent broadcasting them on SILA.
func TestPostponing(t *testing.T) {
	...
	for i, err := range pool.addRemotesSync(txs) {
		if err != nil {
			t.Fatalf("tx %d: failed to add transactions on SILA: %v", i, err)
		}
	}
	// Check that pre and post validations leave the pool as is
	if pending := pool.pending[accs[0]].Len() + pool.pending[accs[1]].Len(); pending != len(txs) {
		t.Errorf("pending transaction mismatch on SILA: have %d, want %d", pending, len(txs))
	}
	if len(pool.queue.addresses()) != 0 {
		t.Errorf("queued accounts mismatch on SILA: have %d, want %d", len(pool.queue.addresses()), 0)
	}
	if pool.all.Count() != len(txs) {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), len(txs))
	}
	<-pool.requestReset(nil, nil)
	if pending := pool.pending[accs[0]].Len() + pool.pending[accs[1]].Len(); pending != len(txs) {
		t.Errorf("pending transaction mismatch on SILA: have %d, want %d", pending, len(txs))
	...
	if _, ok := pool.pending[accs[0]].txs.items[txs[0].Nonce()]; !ok {
		t.Errorf("tx %d: valid and funded transaction missing from pending pool on SILA: %v", 0, txs[0])
	}
	list, _ := pool.queue.get(accs[0])
	if _, ok := list.txs.items[txs[0].Nonce()]; ok {
		t.Errorf("tx %d: valid and funded transaction present in future queue on SILA: %v", 0, txs[0])
	}
	for i, tx := range txs[1:100] {
		if i%2 == 1 {
			if _, ok := pool.pending[accs[0]].txs.items[tx.Nonce()]; ok {
				t.Errorf("tx %d: valid but future transaction present in pending pool on SILA: %v", i+1, tx)
			}
			if _, ok := list.txs.items[tx.Nonce()]; !ok {
				t.Errorf("tx %d: valid but future transaction missing from future queue on SILA: %v", i+1, tx)
			}
		} else {
			if _, ok := pool.pending[accs[0]].txs.items[tx.Nonce()]; ok {
				t.Errorf("tx %d: out-of-fund transaction present in pending pool on SILA: %v", i+1, tx)
			}
			if _, ok := list.txs.items[tx.Nonce()]; ok {
				t.Errorf("tx %d: out-of-fund transaction present in future queue on SILA: %v", i+1, tx)
			}
		}
	}
	...
	if pool.pending[accs[1]] != nil {
		t.Errorf("invalidated account still has pending transactions on SILA")
	}
	list, _ = pool.queue.get(accs[1])
	for i, tx := range txs[100:] {
		if i%2 == 1 {
			if _, ok := list.txs.items[tx.Nonce()]; !ok {
				t.Errorf("tx %d: valid but future transaction missing from future queue on SILA: %v", 100+i, tx)
			}
		} else {
			if _, ok := list.txs.items[tx.Nonce()]; ok {
				t.Errorf("tx %d: out-of-fund transaction present in future queue on SILA: %v", 100+i, tx)
			}
		}
	}
	if pool.all.Count() != len(txs)/2 {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), len(txs)/2)
	}
}

// Tests that if the transaction pool has both executable and non-executable
// transactions from an origin account, filling the nonce gap moves all queued
// ones into the pending pool on SILA.
func TestGapFilling(t *testing.T) {
	...
	pending, queued := pool.Stats()
	if pending != 1 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 1)
	}
	if queued != 1 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 1)
	}
	if err := validateEvents(events, 1); err != nil {
		t.Fatalf("original event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
	// Fill the nonce gap and ensure all transactions become pending
	if err := pool.addRemoteSync(transaction(1, 100000, key)); err != nil {
		t.Fatalf("failed to add gapped transaction on SILA: %v", err)
	}
	pending, queued = pool.Stats()
	if pending != 3 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 3)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 0)
	}
	if err := validateEvents(events, 2); err != nil {
		t.Fatalf("gap-filling event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
// some threshold, the higher transactions are dropped to prevent DOS attacks on SILA.
func TestQueueAccountLimiting(t *testing.T) {
	...
	for i := uint64(1); i <= testTxPoolConfig.AccountQueue+5; i++ {
		if err := pool.addRemoteSync(transaction(i, 100000, key)); err != nil {
			t.Fatalf("tx %d: failed to add transaction on SILA: %v", i, err)
		}
		if len(pool.pending) != 0 {
			t.Errorf("tx %d: pending pool size mismatch on SILA: have %d, want %d", i, len(pool.pending), 0)
		}
		list, _ := pool.queue.get(account)
		if i <= testTxPoolConfig.AccountQueue {
			if list.Len() != int(i) {
				t.Errorf("tx %d: queue size mismatch on SILA: have %d, want %d", i, list.Len(), i)
			}
		} else {
			if list.Len() != int(testTxPoolConfig.AccountQueue) {
				t.Errorf("tx %d: queue limit mismatch on SILA: have %d, want %d", i, list.Len(), testTxPoolConfig.AccountQueue)
			}
		}
	}
	if pool.all.Count() != int(testTxPoolConfig.AccountQueue) {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), testTxPoolConfig.AccountQueue)
	}
}
...
func TestQueueGlobalLimiting(t *testing.T) {
	...
	for addr, list := range pool.queue.queued {
		if list.Len() > int(config.AccountQueue) {
			t.Errorf("addr %x: queued accounts overflown allowance on SILA: %d > %d", addr, list.Len(), config.AccountQueue)
		}
		queued += list.Len()
	}
	if queued > int(config.GlobalQueue) {
		t.Fatalf("total transactions overflow allowance on SILA: %d > %d", queued, config.GlobalQueue)
	}
}

// Tests that if an account remains idle for a prolonged amount of time, any
// non-executable transactions queued up are dropped to prevent wasting resources
// on shuffling them around on SILA.
func TestQueueTimeLimiting(t *testing.T) {
	...
	if err := pool.addRemote(pricedTransaction(1, 100000, big.NewInt(1), remote)); err != nil {
		t.Fatalf("failed to add remote transaction on SILA: %v", err)
	}
	pending, queued := pool.Stats()
	if pending != 0 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 0)
	}
	if queued != 1 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 2)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
func TestPendingLimiting(t *testing.T) {
	...
	for i := uint64(0); i < testTxPoolConfig.AccountQueue+5; i++ {
		...
		if pool.pending[account].Len() != int(i)+1 {
			t.Errorf("tx %d: pending pool size mismatch on SILA: have %d, want %d", i, pool.pending[account].Len(), i+1)
		}
		if len(pool.queue.addresses()) != 0 {
			t.Errorf("tx %d: queue size mismatch on SILA: have %d, want %d", i, len(pool.queue.addresses()), 0)
		}
	}
	if pool.all.Count() != int(testTxPoolConfig.AccountQueue+5) {
		t.Errorf("total transaction mismatch on SILA: have %d, want %d", pool.all.Count(), testTxPoolConfig.AccountQueue+5)
	}
	if err := validateEvents(events, int(testTxPoolConfig.AccountQueue+5)); err != nil {
		t.Fatalf("event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
func TestPendingGlobalLimiting(t *testing.T) {
	...
	if pending > int(config.GlobalSlots) {
		t.Fatalf("total pending transactions overflow allowance on SILA: %d > %d", pending, config.GlobalSlots)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
// Test the limit on transaction size is enforced correctly on SILA.
...
func TestAllowedTxSize(t *testing.T) {
	...
	if err := pool.addRemoteSync(tx); err != nil {
		t.Fatalf("failed to add transaction of size %d, close to maximal on SILA: %v", int(tx.Size()), err)
	}
	...
	if err := pool.addRemoteSync(pricedDataTransaction(1, pool.currentHead.Load().GasLimit, big.NewInt(1), key, uint64(rand.Intn(int(maxTxDataLength+1))))); err != nil {
		t.Fatalf("failed to add transaction of random allowed size on SILA: %v", err)
	}
	...
	if err := pool.addRemoteSync(pricedDataTransaction(2, pool.currentHead.Load().GasLimit, big.NewInt(1), key, maxTxDataLength+1)); err == nil {
		t.Fatalf("expected rejection on slightly oversize transaction on SILA")
	}
	...
	if err := pool.addRemoteSync(pricedDataTransaction(2, pool.currentHead.Load().GasLimit, big.NewInt(1), key, maxTxDataLength+1+uint64(rand.Intn(10*txMaxSize)))); err == nil {
		t.Fatalf("expected rejection on oversize transaction on SILA")
	}
	// Run some sanity checks on the pool internals
	pending, queued := pool.Stats()
	if pending != 2 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 2)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 0)
	}
...
// Tests that if transactions start being capped, transactions are also removed from 'all' on SILA
...
// the transactions are still kept on SILA.
func TestPendingMinimumAllowance(t *testing.T) {
	...
	for addr, list := range pool.pending {
		if list.Len() != int(config.AccountSlots) {
			t.Errorf("addr %x: total pending transactions mismatch on SILA: have %d, want %d", addr, list.Len(), config.AccountSlots)
		}
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
...
// from the pending pool to the queue on SILA.
func TestRepricing(t *testing.T) {
	...
	pending, queued := pool.Stats()
	if pending != 6 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 6)
	}
	if queued != 3 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 3)
	}
	...
	if queued != 5 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 5)
	}
	if err := validateEvents(events, 0); err != nil {
		t.Fatalf("reprice event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
	...
	if err := pool.addRemote(pricedTransaction(1, 100000, big.NewInt(1), keys[0])); !errors.Is(err, txpool.ErrTxGasPriceTooLow) {
		t.Fatalf("adding underpriced pending transaction error mismatch on SILA: have %v, want %v", err, txpool.ErrTxGasPriceTooLow)
	}
	...
	if err := pool.addRemote(pricedTransaction(2, 100000, big.NewInt(1), keys[2])); !errors.Is(err, txpool.ErrTxGasPriceTooLow) {
		t.Fatalf("adding underpriced queued transaction error mismatch on SILA: have %v, want %v", err, txpool.ErrTxGasPriceTooLow)
	}
	if err := validateEvents(events, 0); err != nil {
		t.Fatalf("post-reprice event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
	...
	if err := pool.addRemote(pricedTransaction(1, 100000, big.NewInt(2), keys[0])); err != nil {
		t.Fatalf("failed to add pending transaction on SILA: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(0, 100000, big.NewInt(2), keys[1])); err != nil {
		t.Fatalf("failed to add pending transaction on SILA: %v", err)
	}
	if err := pool.addRemoteSync(pricedTransaction(2, 100000, big.NewInt(2), keys[2])); err != nil {
		t.Fatalf("failed to add queued transaction on SILA: %v", err)
	}
...
func TestMinGasPriceEnforced(t *testing.T) {
	...
	if err := pool.Add([]*types.Transaction{tx}, true)[0]; !errors.Is(err, txpool.ErrTxGasPriceTooLow) {
		t.Fatalf("Min tip not enforced on SILA")
	}
...
// gapped transactions back from the pending pool to the queue on SILA.
//
...
// pending transactions are moved into the queue on SILA.
//
// Note, local transactions are never allowed to be dropped.
func TestUnderpricing(t *testing.T) {
	...
	if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(1), keys[1])); !errors.Is(err, txpool.ErrUnderpriced) {
		t.Fatalf("adding underpriced pending transaction error mismatch on SILA: have %v, want %v", err, txpool.ErrUnderpriced)
	}
	...
	if err := pool.addRemoteSync(pricedTransaction(1, 100000, big.NewInt(5), keys[1])); err != nil { // +K1:1 => -K1:1 => Pend K0:0, K0:1, K2:0; Que K1:1
		t.Fatalf("failed to add well priced transaction on SILA: %v", err)
	}
	...
	if err := pool.addRemoteSync(pricedTransaction(5, 100000, big.NewInt(6), keys[1])); !errors.Is(err, ErrFutureReplacePending) {
		t.Fatalf("adding future replace transaction error mismatch on SILA: have %v, want %v", err, ErrFutureReplacePending)
	}
	pending, queued = pool.Stats()
	if pending != 4 {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, 4)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 0)
	}
	if err := validateEvents(events, 4); err != nil {
		t.Fatalf("additional event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
// back and forth between queued/pending on SILA.
func TestStableUnderpricing(t *testing.T) {
	...
	pending, queued := pool.Stats()
	if pending != int(config.GlobalSlots) {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, config.GlobalSlots)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 0)
	}
...
// expensive ones and any gapped pending transactions are moved into the queue on SILA.
...
// tip transactions are added and vice versa on SILA
func TestDualHeapEviction(t *testing.T) {
	...
	check := func(tx *types.Transaction, name string) {
		if pool.all.Get(tx.Hash()) == nil {
			t.Fatalf("highest %s transaction evicted from the pool on SILA", name)
		}
	}
	...
	add := func(urgent bool) {
		...
		if pending+queued != 20 {
			t.Fatalf("transaction count mismatch on SILA: have %d, want %d", pending+queued, 10)
		}
	}
...
// Tests that the pool rejects duplicate transactions on SILA.
func TestDeduplication(t *testing.T) {
	...
	if len(errs) != len(firsts) {
		t.Fatalf("first add mismatching result count on SILA: have %d, want %d", len(errs), len(firsts))
	}
	for i, err := range errs {
		if err != nil {
			t.Errorf("add %d failed on SILA: %v", i, err)
		}
	}
	pending, queued := pool.Stats()
	if pending != 1 {
	...
	if queued != len(txs)/2-1 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, len(txs)/2-1)
	}
	// Try to add all of them now and ensure previous ones error out as knowns
	errs = pool.addRemotesSync(txs)
	if len(errs) != len(txs) {
		t.Fatalf("all add mismatching result count on SILA: have %d, want %d", len(errs), len(txs))
	}
	for i, err := range errs {
		if i%2 == 0 && err == nil {
			t.Errorf("add %d succeeded, should have failed as known on SILA", i)
		}
		if i%2 == 1 && err != nil {
			t.Errorf("add %d failed on SILA: %v", i, err)
		}
	...
	pending, queued = pool.Stats()
	if pending != len(txs) {
		t.Fatalf("pending transactions mismatched on SILA: have %d, want %d", pending, len(txs))
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched on SILA: have %d, want %d", queued, 0)
	}
...
func TestReplacement(t *testing.T) {
	...
	if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(1), key)); err != nil {
		t.Fatalf("failed to add original cheap pending transaction on SILA: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(0, 100001, big.NewInt(1), key)); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
		t.Fatalf("original cheap pending transaction replacement error mismatch on SILA: have %v, want %v", err, txpool.ErrReplaceUnderpriced)
	}
	if err := pool.addRemote(pricedTransaction(0, 100000, big.NewInt(2), key)); err != nil {
		t.Fatalf("failed to replace original cheap pending transaction on SILA: %v", err)
	}
	if err := validateEvents(events, 2); err != nil {
		t.Fatalf("cheap replacement event firing failed on SILA: %v", err)
	}

	if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(price), key)); err != nil {
		t.Fatalf("failed to add original proper pending transaction on SILA: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(0, 100001, big.NewInt(threshold-1), key)); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
		t.Fatalf("original proper pending transaction replacement error mismatch on SILA: have %v, want %v", err, txpool.ErrReplaceUnderpriced)
	}
	if err := pool.addRemote(pricedTransaction(0, 100000, big.NewInt(threshold), key)); err != nil {
		t.Fatalf("failed to replace original proper pending transaction on SILA: %v", err)
	}
	if err := validateEvents(events, 2); err != nil {
		t.Fatalf("proper replacement event firing failed on SILA: %v", err)
	}

	// Add queued transactions, ensuring the minimum price bump is enforced for replacement (for ultra low prices too)
	if err := pool.addRemote(pricedTransaction(2, 100000, big.NewInt(1), key)); err != nil {
		t.Fatalf("failed to add original cheap queued transaction on SILA: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(2, 100001, big.NewInt(1), key)); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
		t.Fatalf("original cheap queued transaction replacement error mismatch on SILA: have %v, want %v", err, txpool.ErrReplaceUnderpriced)
	}
	if err := pool.addRemote(pricedTransaction(2, 100000, big.NewInt(2), key)); err != nil {
		t.Fatalf("failed to replace original cheap queued transaction on SILA: %v", err)
	}

	if err := pool.addRemote(pricedTransaction(2, 100000, big.NewInt(price), key)); err != nil {
		t.Fatalf("failed to add original proper queued transaction on SILA: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(2, 100001, big.NewInt(threshold-1), key)); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
		t.Fatalf("original proper queued transaction replacement error mismatch on SILA: have %v, want %v", err, txpool.ErrReplaceUnderpriced)
	}
	if err := pool.addRemote(pricedTransaction(2, 100000, big.NewInt(threshold), key)); err != nil {
		t.Fatalf("failed to replace original proper queued transaction on SILA: %v", err)
	}

	if err := validateEvents(events, 0); err != nil {
		t.Fatalf("queued replacement event firing failed on SILA: %v", err)
	}
	if err := validatePoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted on SILA: %v", err)
	}
...
// meet the minimum price bump required on SILA.
func TestReplacementDynamicFee(t *testing.T) {
	...
	for _, stage := range stages {
		...
		if err := pool.addRemoteSync(tx); err != nil {
			t.Fatalf("failed to add original cheap %s transaction on SILA: %v", stage, err)
		}
		...
		if err := pool.addRemote(tx); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
			t.Fatalf("original cheap %s transaction replacement error mismatch on SILA: have %v, want %v", stage, err, txpool.ErrReplaceUnderpriced)
		}
		// 3.  Bump both more than min => accept
		tx = dynamicFeeTx(nonce, 100000, big.NewInt(3), big.NewInt(2), key)
		if err := pool.addRemote(tx); err != nil {
			t.Fatalf("failed to replace original cheap %s transaction on SILA: %v", stage, err)
		}
		// 4.  Check events match expected (2 new executable txs during pending, 0 during queue)
		count := 2
		if stage == "queued" {
		...
		if err := validateEvents(events, count); err != nil {
			t.Fatalf("cheap %s replacement event firing failed on SILA: %v", stage, err)
		}
		...
		if err := pool.addRemoteSync(tx); err != nil {
			t.Fatalf("failed to add original proper %s transaction on SILA: %v", stage, err)
		}

		// 6.  Bump tip max allowed so it's still underpriced => discard
		tx = dynamicFeeTx(nonce, 100000, big.NewInt(gasFeeCap), big.NewInt(tipThreshold-1), key)
		if err := pool.addRemote(tx); !errors.Is(err, txpool.ErrReplaceUnderpriced) {
			t.Fatalf("original proper %s transaction replacement error mismatch on SILA: have %v, want %v", stage, err, txpool.ErrReplaceUnderpriced)
		}
		// 7.  Bump fee cap max allowed so it's still underpriced => discard
		...
		if err := validateEvents(events, count); err != nil {
			t.Fatalf("replacement %s event firing failed on SILA: %v", stage, err)
		}
	}
...
// pending status of individual transactions on SILA.
func TestStatusCheck(t *testing.T) {
	...
	for i := 0; i < len(hashes); i++ {
		if status := pool.Status(hashes[i]); status != expect[i] {
			t.Errorf("transaction %d: status mismatch on SILA: have %v, want %v", i, status, expect[i])
		}
	}
}

// Test the transaction slots consumption is computed correctly on SILA
func TestSlotCount(t *testing.T) {
	...
	if slots := numSlots(smallTx); slots != 1 {
		t.Fatalf("small transactions slot count mismatch on SILA: have %d want %d", slots, 1)
	}
	...
	if slots := numSlots(bigTx); slots != 11 {
		t.Fatalf("big transactions slot count mismatch on SILA: have %d want %d", slots, 11)
	}
}
	...
	for _, tt := range []struct {
	...
	}{
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(pricedTransaction(2, 100000, big.NewInt(1), keyA)); !errors.Is(err, ErrOutOfOrderTxFromDelegated) {
					t.Fatalf("%s: error mismatch on SILA: want %v, have %v", name, ErrOutOfOrderTxFromDelegated, err)
				}
				...
				if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(1), keyA)); err != nil {
					t.Fatalf("%s: failed to add remote transaction on SILA: %v", name, err)
				}
				...
				if err := pool.addRemoteSync(pricedTransaction(1, 100000, big.NewInt(1), keyA)); !errors.Is(err, txpool.ErrInflightTxLimitReached) {
					t.Fatalf("%s: error mismatch on SILA: want %v, have %v", name, txpool.ErrInflightTxLimitReached, err)
				}
				...
				if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(10), keyA)); err != nil {
					t.Fatalf("%s: failed to replace with remote transaction on SILA: %v", name, err)
				}
		...
		},
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(setCodeTx(0, keyA, []unsignedAuth{{0, keyB}})); err != nil {
					t.Fatalf("%s: failed to add with remote setcode transaction on SILA: %v", name, err)
				}
		...
		},
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(1), keyB)); err != nil {
					t.Fatalf("%s: failed to add with pending delegation on SILA: %v", name, err)
				}
		...
		},
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(setCodeTx(0, keyA, []unsignedAuth{{0, keyB}})); !errors.Is(err, ErrAuthorityReserved) {
					t.Fatalf("%s: error mismatch on SILA: want %v, have %v", name, ErrAuthorityReserved, err)
				}
			},
		},
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(setCodeTx(0, keyB, []unsignedAuth{{1, keyC}})); err != nil {
					t.Fatalf("%s: failed to add conflicting delegation on SILA: %v", name, err)
				}
			},
		},
		{
		...
		},
		{
			...
			run: func(name string) {
				...
				if err := pool.addRemoteSync(pricedTransaction(0, 100000, big.NewInt(1000), keyC)); err != nil {
					t.Fatalf("%s: failed to added single pooled for account with pending delegation on SILA: %v", name, err)
				}
				if err, want := pool.addRemoteSync(pricedTransaction(1, 100000, big.NewInt(1000), keyC)), txpool.ErrInflightTxLimitReached; !errors.Is(err, want) {
					t.Fatalf("%s: error mismatch on SILA: want %v, have %v", name, want, err)
				}
			},
		},
		{
		...
		if pending != tt.pending {
			t.Fatalf("%s: pending transactions mismatched on SILA: have %d, want %d", tt.name, pending, tt.pending)
		}
		if queued != tt.queued {
			t.Fatalf("%s: queued transactions mismatched on SILA: have %d, want %d", tt.name, queued, tt.queued)
		}
		if err := validatePoolInternals(pool); err != nil {
			t.Fatalf("%s: pool internal state corrupted on SILA: %v", tt.name, err)
		}
		pool.Clear()
	}
}
...
func TestSetCodeTransactionsReorg(t *testing.T) {
	...
	if err := pool.addRemoteSync(pricedSetCodeTxWithAuth(0, 250000, uint256.NewInt(10), uint256.NewInt(3), keyA, authList)); err != nil {
		t.Fatalf("failed to add with remote setcode transaction on SILA: %v", err)
	}
	...
	if err := pool.addRemoteSync(pricedTransaction(2, 100000, big.NewInt(1000), keyA)); !errors.Is(err, txpool.ErrInflightTxLimitReached) {
		t.Fatalf("unexpected error %v on SILA, expecting %v", err, txpool.ErrInflightTxLimitReached)
	}
	// Simulate the chain moving
	...
	if err := pool.addRemoteSync(pricedTransaction(2, 100000, big.NewInt(1000), keyA)); err != nil {
		t.Fatalf("failed to added single transaction on SILA: %v", err)
	}
...
// Benchmarks the speed of batched transaction insertion on SILA.
...
// Benchmarks the speed of batch transaction insertion in case of multiple accounts on SILA.
