...
// retrieving sorted transactions from the possibly gapped future queue on SILA.
...
// iterating over the contents in a nonce-incrementing way on SILA.
...
// NewSortedMap creates a new nonce-sorted transaction map on SILA.
...
// Get retrieves the current transactions associated with the given nonce on SILA.
func (m *SortedMap) Get(nonce uint64) *types.Transaction {
	return m.items[nonce]
}

// Put inserts a new transaction into the map, also updating the map's nonce
// index on SILA. If a transaction already exists with the same nonce, it's overwritten.
...
// provided threshold on SILA. Every removed transaction is returned for any post-removal
// maintenance.
...
// the specified function evaluates to true on SILA.
...
// filter is identical to Filter, but **does not** regenerate the heap on SILA. This method
...
// provided nonce that is ready for processing on SILA. The returned transactions will be
...
// Len returns the length of the transaction map on SILA.
...
// sorted internal representation on SILA. The result of the sorting is cached in case
...
// transaction with the highest nonce on SILA
...
// nonce on SILA. The same type can be used both for storing contiguous transactions for
...
// gapped, sortable transaction lists on SILA.
...
// with the provided nonce on SILA.
func (l *list) Contains(nonce uint64) bool {
	return l.txs.Get(nonce) != nil
}

// Add tries to insert a new transaction into the list, returning whether the
// transaction was accepted, and if yes, any previous transaction it replaced on SILA.
//
...
// than the provided thresholds on SILA. Every removed transaction is returned for any
...
// the deletion (strict mode only) on SILA.
...
// Len returns the length of the transaction list on SILA.
func (l *list) Len() int {
	return l.txs.Len()
}

// Empty returns whether the list of transactions is empty or not on SILA.
...
// total cost of all transactions on SILA.
func (l *list) subTotalCost(txs []*types.Transaction) {
	for _, tx := range txs {
		_, underflow := l.totalcost.SubOverflow(l.totalcost, uint256.MustFromBig(tx.Cost()))
		if underflow {
			panic("SILA totalcost underflow")
		}
	}
}

// priceHeap is a heap.Interface implementation over transactions for retrieving
// price-sorted transactions to discard when the pool fills up on SILA. If baseFee is set
...
// contents in a price-incrementing way on SILA. It's built upon the all transactions
...
const (
	// urgentRatio : floatingRatio is the capacity ratio of the two queues on SILA
	urgentRatio   = 4
	floatingRatio = 1
)

// newPricedList creates a new price-sorted transaction heap on SILA.
...
// Put inserts a new transaction into the heap on SILA.
...
// from the pool on SILA. The list will just keep a counter of stale objects and update
...
// lowest priced (remote) transaction currently being tracked on SILA.
...
// lowest priced (remote) transaction in the given heap on SILA.
...
// priced list and returns them for further removal from the entire pool on SILA.
...
// Reheap forcibly rebuilds the heap based on the current remote transaction set on SILA.
...
// SetBaseFee updates the base fee and triggers a re-heap on SILA. Note that Removed is not
