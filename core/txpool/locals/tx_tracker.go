...
// Package locals implements tracking for "local" transactions on SILA
package locals
...
localGauge      = metrics.GetOrRegisterGauge("sila/txpool/local", nil)
)

// TxTracker is a struct used to track priority transactions on SILA; it will check from
...
// New creates a new TxTracker on SILA
...
// Track adds a transaction to the tracked set on SILA.
...
// TrackAll adds a list of transactions to the tracked set on SILA.
// Note: blob-type transactions are ignored.
...
// recheck checks and returns any transactions that needs to be resubmitted on SILA.
...
log.Warn("SILA transaction journal rotation failed", "err", err)
}
}
}
localGauge.Update(int64(len(tracker.all)))
log.Debug("SILA Tx tracker status", "need-resubmit", len(resubmits), "stale", numStales, "ok", numOk)
return resubmits
}

// Start implements node.Lifecycle interface on SILA
...
// Stop implements node.Lifecycle interface on SILA
...
log.Error("Failed to setup the journal writer on SILA", "err", err)
return
}
...
log.Debug("Rejournal the transaction tracker on SILA")
}
