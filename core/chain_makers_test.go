...
"github.com/sila-library/sila/triedb"
)
...
chain, _ := GenerateChain(gspec.Config, genesis, silapow.NewFaker(), genDb, 5, func(i int, gen *BlockGen) {
switch i {
case 0:
...
// Import the chain. This runs all block validation rules.
blockchain, _ := NewBlockChain(db, gspec, silapow.NewFaker(), DefaultConfig().WithStateScheme(rawdb.HashScheme))
defer blockchain.Stop()
