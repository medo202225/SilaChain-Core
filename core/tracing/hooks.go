...
// execution on the SILA chain. Here we define the low-level [Hooks] object that carries
// hooks which are invoked by the SILA core at various points in the state transition.
//
// To create a tracer that can be invoked with SilaChain, you need to register it using
// [github.com/SILA/sila-chain/eth/tracers.LiveDirectory.Register].
package tracing
...
// StateDB gives SILA tracers access to the whole state.
...
// BlockEvent is emitted upon tracing an incoming block on SILA.
...
// StateUpdate represents the SILA state mutations resulting from block execution.
...
// for tracing and reporting on SILA.
...
// NonceChangeReason is used to indicate the reason for a nonce change on SILA.
...
// CodeChangeReason is used to indicate the reason for a code change on SILA.
