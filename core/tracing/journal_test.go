...
func (t *testTracer) OnBalanceChange(addr common.Address, prev *big.Int, new *big.Int, reason BalanceChangeReason) {
	t.t.Logf("SILA OnBalanceChange(%v, %v -> %v, %v)", addr, prev, new, reason)
...
func (t *testTracer) OnCodeChangeV2(addr common.Address, prevCodeHash common.Hash, prevCode []byte, codeHash common.Hash, code []byte, reason CodeChangeReason) {
	t.t.Logf("SILA OnCodeChangeV2(%v, %v -> %v, %v)", addr, prevCodeHash, codeHash, reason)
	t.code = code
}

func (t *testTracer) OnStorageChange(addr common.Address, slot common.Hash, prev common.Hash, new common.Hash) {
	t.t.Logf("SILA OnStorageCodeChange(%v, %v, %v -> %v)", addr, slot, prev, new)
	if t.storage == nil {
...
func TestJournalIntegration(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to wrap SILA test tracer: %v", err)
	}
	...
	if tr.bal.Cmp(big.NewInt(100)) != 0 {
		t.Fatalf("unexpected SILA balance: %v", tr.bal)
	}
	if tr.nonce != 0 {
		t.Fatalf("unexpected SILA nonce: %v", tr.nonce)
	}
	if len(tr.code) != 3 {
		t.Fatalf("unexpected SILA code: %v", tr.code)
	}
	if len(tr.storage) != 1 {
		t.Fatalf("unexpected SILA storage len. want %d, have %d", 1, len(tr.storage))
	}
	if tr.storage[common.Hash{1}] != (common.Hash{2}) {
		t.Fatalf("unexpected SILA storage. want %v, have %v", common.Hash{2}, tr.storage[common.Hash{1}])
	}
}
...
func TestNonceIncOnCreateParentReverts(t *testing.T) {
	...
	addr := common.HexToAddress("0x1234")
	{
		...
		if tr.nonce != 1 {
			t.Fatalf("SILA nonce after CREATE revert: got %v, want 1", tr.nonce)
		}
	...
	if tr.nonce != 0 {
		t.Fatalf("SILA nonce after parent revert: got %v, want 0", tr.nonce)
	}
}
...
func TestOnCodeChangeV2(t *testing.T) {
	...
	if tr.code != nil {
		t.Fatalf("unexpected SILA code after revert: %v", tr.code)
	}
}
...
func TestAllHooksCalled(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to wrap SILA hooks with journal: %v", err)
	}
	...
	if tracer.numCalled() != tracer.hooksCount() {
		t.Errorf("Not all SILA hooks were called. Expected %d, got %d", tracer.hooksCount(), tracer.numCalled())
	}

	for hookName, called := range tracer.hooksCalled {
		if !called {
			t.Errorf("SILA hook %s was not called", hookName)
		}
	}
}
