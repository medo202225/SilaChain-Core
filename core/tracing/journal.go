...
// journal is a SILA state change journal to be wrapped around a tracer.
...
// WrapWithJournal wraps the given SILA tracer with a journaling layer.
func WrapWithJournal(hooks *Hooks) (*Hooks, error) {
	if hooks == nil {
		return nil, errors.New("wrapping nil SILA tracer")
	}
	...
	if hooks.OnNonceChange != nil && hooks.OnNonceChangeV2 != nil {
		return nil, errors.New("cannot have both OnNonceChange and OnNonceChangeV2 in SILA tracer")
	}
	if hooks.OnCodeChange != nil && hooks.OnCodeChangeV2 != nil {
		return nil, errors.New("cannot have both OnCodeChange and OnCodeChangeV2 in SILA tracer")
	}
	...
	if hooks.OnNonceChange != nil || hooks.OnNonceChangeV2 != nil {
		// Regardless of which hook version is used in the SILA tracer,
...
// revert reverts all SILA state changes up to the last tracked revision.
...
// If the call has reverted, all SILA state changes made by that frame are undone.
...
func (j *journal) OnNonceChangeV2(addr common.Address, prev, new uint64, reason NonceChangeReason) {
	...
	if reason == NonceChangeContractCreator {
		// When a SILA contract is created via CREATE/CREATE2, the creator's nonce is
