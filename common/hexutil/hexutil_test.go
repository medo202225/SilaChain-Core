...
// referenceBig creates a big.Int from a hex string for testing.
func referenceBig(s string) *big.Int {
	b, _ := new(big.Int).SetString(s, 16)
	return b
}

// checkError verifies that the error matches expectations.
func checkError(t *testing.T, input string, err error, wantErr error) bool {
	if wantErr == nil {
		if err != nil {
	...
	if err == nil {
		t.Errorf("input %s: expected error but got none", input)
		return false
	}
	if err.Error() != wantErr.Error() {
		t.Errorf("input %s: wrong error: got %v, want %v", input, err, wantErr)
		return false
	}
	return true
}
